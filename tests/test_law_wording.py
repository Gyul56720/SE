...
print()
print("[표제는 규정이 아니다] **표제어 하나로 문서의 낱말을 기각하지 않는다**")
# 실측: 민사소송법 제203조는 표제가 (처분권주의) 이고 본문은 "법원은 당사자가 신청하지
# 아니한 사항에 대하여는 판결하지 못한다" 뿐이다. 그런데 문서가 '변론주의' 를 쓰자
# "조문은 '처분권주의' 인데 바꿔 적었다" 며 hard 로 기각했다 -- 그 낱말은 조문 본문에
# 한 번도 안 나온다.
#
# **처음엔 이것을 '유보 문장은 대조하지 않는다' 로 고쳤는데 그게 틀렸다.** 문장을 통째로
# 침묵시키니 돌연변이 셋이 그 그늘로 숨었다(실측: "제355조의 **절도죄** 관련 ... 명시되지
# 않음" 이 안 걸렸다). 한 문장은 유보와 주장을 같이 담을 수 있다.
_표제 = "제203조(처분권주의) 법원은 당사자가 신청하지 아니한 사항에 대하여는 판결하지 못한다."
ok(WD._term_rows("변론주의에 관하여는 제203조 원문에 명시되지 않음.", _표제)
   == [("용어:변론주의", "견줄것없음")],
   f"표제에만 있는 낱말로는 기각하지 않는다 "
   f"(얻은 값 {WD._term_rows('변론주의에 관하여는 제203조 원문에 명시되지 않음.', _표제)})")
# **맞음에는 표제도 근거다.** 기각은 무겁고 인정은 가벼우므로 무거운 쪽에만 더 센 근거를
# 요구한다 -- 과잉 기각하는 심판은 맞는 답도 버린다.
ok(("용어:처분권주의", "맞음") in WD._term_rows("제203조는 처분권주의를 정한다.", _표제),
   "문서가 표제어를 쓴 것은 근거가 있다 -- 그쪽은 맞음으로 본다")
# **유보 문장 안에 든 낱말 바꿔치기는 그대로 잡혀야 한다** -- 돌연변이가 심는 꼴이다.
_본문 = ("제355조(횡령, 배임) 타인의 재물을 보관하는 자가 그 재물을 횡령하거나 "
       "그 반환을 거부한 때에는 5년 이하의 징역에 처한다.")
ok(("용어:절도(조문은 횡령)", "어긋남") in WD._term_rows(
       "제355조의 본문 내용 중 절도죄와 관련된 구성요건은 조문 원문에 "
       "명시되지 않음, 학설/판례 확인 필요합니다.", _본문),
   "'명시되지 않음' 이 붙어 있어도 본문의 낱말을 바꿔 적으면 잡는다  <- 돌연변이가 심는 꼴")

# **표제를 옮겨 적으면서 틀린 것은 잡아야 한다.** 표제를 통째로 빼니 이 자리가
# 그늘로 들어갔다(실측: `형법 제30조(방조)는 ...` -- 제30조의 표제는 (공동정범)이고
# 그 낱말은 본문에 없다). 가르는 것은 `제N조(...)` 꼴 하나다 -- 표제를 옮겨 적었는가.
_제30 = "제30조(공동정범) 2인 이상이 공동하여 죄를 범한 때에는 각자를 그 죄의 정범으로 처벌한다."
ok(("용어:방조(조문은 공동정범)", "어긋남") in WD._term_rows(
       '형법 제30조(방조)는 "2인 이상이 공동하여 죄를 범한 때에는 각자를 그 죄의 '
       '정범으로 처벌한다"고 규정하고 있습니다.', _제30),
   "표제를 옮겨 적으면서 틀리면 잡는다  <- 표제를 뺐다가 여기가 그늘이 됐다")
ok(WD._term_rows("공동정범과 방조는 구별하여야 한다. 제30조 참조.", _제30)
   [0][1] != "어긋남",
   "표제를 옮겨 적지 않은 산문은 그대로 둔다 -- 가르는 것은 `제N조(...)` 꼴이다")
