...
print()
print("[유보] **모른다고 밝힌 문장은 조문에 관한 주장이 아니다**")
# 실측: 민사소송법 제203조 편에서 이 문장이 W005 hard 로 기각됐다. 그런데 문서는 옳다 --
# 바로 앞 문장에서 "이를 처분권주의라 합니다" 라고 제대로 적었고, 이 문장은 변론주의가
# 그 조문에 **없다고 밝힌** 것이다. 조문의 낱말을 바꿔 적은 것이 아니다.
#
# 이건 그냥 오탐이 아니라 **닫힌책 규율을 지킨 자리를 벌한 것**이다. 생성자에게 시킨
# 바로 그 일을 하면 관문이 기각하고, 그러면 다음 원고는 유보를 안 쓰는 쪽으로 간다
# -- 관문이 환각을 권하는 꼴이다.
# 시금석 제30조는 "각하한다" 라 쓴다. 유보를 안 빼면 이 문장이 "'각하' 를
# '기각' 으로 바꿔 적었다" 로 기각된다 -- 실측의 처분권주의/변론주의와 같은 꼴이다.
_유보 = "기각 여부에 관하여는 제30조 원문에 명시되지 않음, 학설/판례 확인 필요."
r = row(_유보)
ok(r and r["오해"] and not r["어긋남"],
   f"'명시되지 않음, 확인 필요' 는 대조하지 않는다 (얻은 값 어긋남 {r and r['어긋남']})")
ok(not WD.check(doc(_유보), CORPUS),
   "관문도 그 자리를 안 잡는다 -- 보고와 판정이 갈라지지 않는다")
# **여기서 멈춰야 한다.** 조문에 없는 것을 있다고 적으면 그건 유보가 아니라 주장이다.
r = row("제20조는 행정청이 시정을 명하여야 한다고 규정한다.")
ok(r and r["어긋남"] == ["서법:기속"],
   f"유보 없이 단정하면 그대로 어긋남이다 (얻은 값 {r and r['어긋남']})")
