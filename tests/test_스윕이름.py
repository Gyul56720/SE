...
# `자원` 은 **준 제약**, `연산기` 는 **실제로 잡힌 수**다. 식이 `(a0*x0)+(a1*x1)`
# 이라 곱셈이 둘뿐이므로 제약을 4로 줘도 둘만 잡힌다 (실측으로 확인했다).
_t = [{"이름": "4곱셈기", "자원": {"mul": 4, "add": 2}, "연산기": {"mul": 2, "add": 1},
      "지연": 3, "II": 1, "면적추정": 2456},
     {"이름": "2곱셈기", "자원": {"mul": 2, "add": 1}, "연산기": {"mul": 2, "add": 1},
      "지연": 3, "II": 1, "면적추정": 2456},
     {"이름": "1곱셈기", "자원": {"mul": 1, "add": 1}, "연산기": {"mul": 1, "add": 1},
      "지연": 5, "II": 2, "면적추정": 1916}]
_맞 = 민글(RA._맞바꿈글(_t))
...
# ============================================ 5b. **준 것과 쓴 것을 가른다**
# 실측 2026-09-23: 표의 「곱셈기」 칸이 `x["자원"]["mul"]` -- **제약**이었다.
# 그래서 「곱셈기 4 · 면적 2,456」과 「곱셈기 2 · 면적 2,456」이 나란히 서서,
# **면적 모형이 곱셈기 수를 안 본다**고 읽히게 했다. 모형은 멀쩡했다 --
# 바인딩이 식에 든 곱셈만큼만 잡으므로 둘은 **같은 하드웨어**다.
ok("제약" in _맞 and "실제로 잡힌 곱셈기는 2 → 1개" in _맞,
   f"**맞바꿈 설명이 「준 것」과 「쓴 것」을 둘 다 말한다** ({_맞[_맞.find('실제로'):][:34]})")
ok("제약 4 → 쓴 것 2" in _파,
   f"**파레토 설명이 왜 같은지를 수로 댄다** ({_파[_파.find('제약 4'):][:26]})")

from house import hls as HLS                                 # noqa: E402

_식 = "(a0*x0) + (a1*x1)"
_g = HLS.읽기(_식)
_넉넉 = HLS.바인딩(_g, {"mul": 4, "add": 2, "sub": 2})
_g2 = HLS.읽기(_식)
HLS.스케줄(_g2, {"mul": 2, "add": 1, "sub": 1})
_딱 = HLS.바인딩(_g2, {"mul": 2, "add": 1, "sub": 1})
ok(_넉넉["연산기수"]["mul"] == 2,
   f"**제약 4 를 줘도 곱셈기는 2개만 잡힌다** ({_넉넉['연산기수']}) — "
   f"식에 곱셈이 둘뿐이다")
ok(_넉넉["총면적_추정"] == _딱["총면적_추정"],
   f"그래서 면적이 같다 ({_넉넉['총면적_추정']:,.0f}) — **모형이 곱셈기 수를 "
   f"안 보는 것이 아니다**")
_g3 = HLS.읽기(_식)
HLS.스케줄(_g3, {"mul": 1, "add": 1, "sub": 1})
_하나 = HLS.바인딩(_g3, {"mul": 1, "add": 1, "sub": 1})
ok(_하나["총면적_추정"] < _넉넉["총면적_추정"],
   f"**진짜로 줄이면 면적이 준다** ({_하나['총면적_추정']:,.0f} < "
   f"{_넉넉['총면적_추정']:,.0f}) — 모형은 쓴 연산기를 센다")
...
for 없어야, 뭐 in (
    ...
    ('x["자원"]["add"], x["지연"]', "표에서 제약을 「덧셈기」로 찍던 자리"),
