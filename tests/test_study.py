...
print()
print("── 오답인가 모름인가 -- **꼭 묻는다** ───────────────────")
n4 = 공책({"q1": False, "q2": False}, {"q1": ["가"], "q2": ["가"]})
ok(len(n4.안물어본것()) == 2, "**틀렸는데 안 물어본 것을 센다** -- 여기가 다음에 할 일")
n4.시도[0].갈래 = "오답"
ok(len(n4.안물어본것()) == 1 and len(n4.사유없는것()) == 1,
   "갈래만 정해지고 사유가 없으면 따로 센다 -- 셈에 안 들어간다")
n4.시도[0].사유 = "분모를 전체로 잡는다"
ok(not n4.사유없는것() or len(n4.사유없는것()) == 0, "사유까지 붙으면 빠진다")
...
for a in 한가지.시도:
    a.갈래, a.사유 = "오답", "늘 같은 것"
ok(all(w.판정 == "못잼" for w in WK.사유취약점(한가지, "오답")),
...
with tempfile.TemporaryDirectory() as d:
    ...
    ok("1) 오답" in out and "2) 모름" in out and "--갈래" in out,
       "**틀리면 오답인지 모름인지 1·2 로 고르라고 묻는다**")
    ...
    code, out = run(["--공책", 자리, "--갈래", "q1", "1", "분모를 전체로 잡는다"])
    ok(code == 0 and "오답" in out and "분모" in out, "**1 로 오답을 고른다**")
    code, out = run(["--공책", 자리, "--갈래", "q1", "9"])
    ok(code == 1 and "1 이나 2 로 골라라" in out, "1·2 밖은 안 받는다")
    code, out = run(["--공책", 자리, "--갈래", "없는id", "1"])
...
print("채점(모르면 None) · 이항 꼬리 · 짧으면 못잼 · 차면 약함 · "
      ...
      "오답/모름을 꼭 물음 · 취약점이 과목이 아니라 어긋난 자리 -- 통과")
