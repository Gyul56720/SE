...
class Fake:
    ...
    def __call__(self, prompt):
        ...
        if "서브플롯 한 씬" in prompt:               # 서브플롯도 창작이다 -> Markdown
            ...
            # **매번 다른 소재를 낸다.** 같은 것을 반복하면 _too_similar 가 되돌려보내고
            # 서브플롯이 0개가 된다 -- 실제 모델에 요구하는 것을 가짜도 지켜야 한다
            # (2026-09-04: 진짜 런에서 세 회차가 같은 소재였던 것을 고치면서 넣은 검사다).
            n = self.stages["subplot"]
            소재 = ["정우가 오디션에서 떨어진다", "재현이 지휘 수업에서 쫓겨난다",
                    "도영이 조교실 불을 늦게까지 켜둔다", "설윤이 기숙사 짐을 싼다",
                    "공명이 형의 악보를 태우려다 만다", "정우가 학식에서 자리를 맡는다",
                    "재현이 낡은 메트로놈을 고친다", "설윤이 지하철 막차를 놓친다",
                    "도영이 유학 지원서를 반려당한다", "공명이 빈 강의실 피아노를 닫는다",
                    "정우가 편곡을 통째로 지운다", "재현이 병원 로비에서 기다린다",
                    "설윤이 알바를 하루 빠진다", "도영이 추천서를 다시 쓴다",
                    "공명이 연습실 열쇠를 반납한다"]
            return (f"## 장면\n{소재[(n - 1) % len(소재)]}\n\n"
                    "## 등장인물\n설윤, 정우\n\n"
                    "## 공간\n복도 끝 게시판 앞\n\n"
