...
print()
print("[무대 낱말] **우리가 쓰는 말이 원고에 나오면 안 된다**")
# 실측 2026-09-09, 사용자 원고 1,115자에 "화자" 가 **일곱 번** 나왔다. 프롬프트가
# 주인공을 그렇게 부르는데(집필 프롬프트에만 여섯 번), 이름이 없으니 모델이 그것을
# 이름으로 썼다. 디렉터가 칸 설명을 값 대신 베끼던 것(beat.echoed)과 같은 병인데
# 원고 쪽에는 그 자가 없었다.
_leak = "밑바닥 구역에서 눈을 뜬 화자가 인장을 안고 걸었다. 결코 멈출 줄 모르는 화자의 걸음."
ok(echo.stage(_leak) == [("화자", 2)], f"새어 나온 낱말과 횟수를 센다 ({echo.stage(_leak)})")
ok(any("무대 뒤의 낱말" in c for c in echo.check(_leak, "")), "check 가 그것을 말한다")
ok(any("이름이나 호칭으로 불러라" in c for c in echo.check(_leak, "")), "무엇을 하라고 말한다")
ok(echo.stage(ONCE) == [], f"멀쩡한 글은 조용하다 ({echo.stage(ONCE)})")
# **흔한 낱말은 안 넣는다** -- 늑대소년이 되면 진짜 새는 자리가 묻힌다.
for _w in ("주인공", "쾌감", "장면", "설정"):
    ok(_w not in echo.STAGE, f"  '{_w}' 는 목록에 없다  ← 원고에 나올 수 있는 말이다")

print()
if fails:
    print(f"메아리: {len(fails)}개 실패 -- {fails}")
    sys.exit(1)
print("메아리: 검출 · 관용 · 꼬리 절단 · 지우기 · 무대 낱말 · 하드 개입 · 과다 호명 · 라벨 -- 통과")
