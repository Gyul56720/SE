"""도구 규칙 검사(tools/lint_tooling.py)가 **실제로 실패하는가.**

실패해야 할 때 실패하지 못하는 검사는 아무 소용이 없다. 통과만 하는 lint 는 아무것도 지키지 않으면서 지키고
있다는 착각을 준다 -- 이 저장소에서 하루 종일 고친 병이 정확히 그것이다(전부 통과시키는 심판).

그래서 규칙마다 **그 규칙이 막으려던 버그를 되살려** 실패하는지 본다. 되살리는 방법은
그 버그를 고친 커밋을 되돌리는 것과 같은 한 줄 치환이다.
  ...
  키-쌍        _FALLBACK 을 지운다       -> 내 컨테이너에서만 통과하던 시절
  마감-실측     TIMEOUT 을 60 으로       -> 504 를 우리가 만들던 시절
  예산-일치     기술서 예산을 손으로 적는다 -> 기술서와 실제가 조용히 어긋나는 자리
  계산-강제     계산 검사를 뗀다          -> 다섯 판 내리 상수로 적은 코드가 채택되던 시절
  미지-범주     미분류/상수로 적은 코드를 지운다     -> 하드코딩을 새 방법이라 부르던 시절
  구간-네자리   at_upper 를 지운다        -> 27 을 "구간 [19,23] 안"이라 하던 시절
  상태코드-분기 상태 분기를 지운다        -> 전부 "구글 쪽 문제"이던 시절
  버전-보존     _snapshot_code 를 지운다  -> 수리가 이전 판을 덮어쓰던 시절
  심판-무해     verify.py 에 lstsq 호출을 넣는다 -> 심판이 답을 담던 시절

그런데 **손으로 쓴 변형만으로는 부족하다.** 그것은 "내가 고른 그 변형에 걸린다"만
증명한다. 그래서 변형을 규칙에서 유도하는 검사를 하나 더 둔다 -- require 패턴에 걸리는
모든 자리를 지우면 규칙은 반드시 실패해야 한다. 실패하지 않으면 그 패턴이 주석이나 정의부에
걸려 있다는 뜻이다.

첫 판에서 넷(RPM-분리 · 미지-범주 · 상태코드-분기 · 심판-무해)이 실패하지 못했다.
셋은 규칙이 OR 라서 하나만 남겨도 통과했고 하나는 되살리는 방법이 약했다. **이 검사가
없었으면 아무것도 걸러내지 못하는 규칙 넷을 통과로 착각했을 것이다** -- 전부 통과시키는 심판과 같은 병이다.
"""
...
MUTATIONS = [
    ...
    ("파이썬-건강", "orchestrator/solve.py",
     "def failure_reasons(run_dir) -> list:",
     "def drive():\n    pass\n\n\ndef failure_reasons(run_dir) -> list:"),
    ("주입-기억", "orchestrator/problems/tensor_rank/run.py",
     "orch_solve.remember_verifier(run_dir, rel)", "pass"),
    ("미지-범주", "orchestrator/method_trace.py", "알려진 방식 밖", "unknown_kind"),
    ("구간-네자리", "orchestrator/problems/tensor_rank/verify.py",
     '"at_upper"', '"inside2"'),
    ("상태코드-분기", "orchestrator/probe_gemini.py", "st in (", "st_was in ("),
    ("버전-보존", "orchestrator/planner.py", "_snapshot_code(", "_gone_snapshot("),
...
def test_clean_repo_is_green() -> None:
    """지금 저장소는 통과해야 한다. 늘 실패하는 검사는 곧 무시된다."""
    bad = lint.run()
    check(not bad, "깨끗한 저장소에서 위반이 나온다: " +
          "; ".join(f"{r['id']}: {m}" for r, m in bad[:4]))
    print(f"    [통과] 규칙 {len(lint.RULES)}개 통과")


def test_every_rule_can_go_red() -> None:
    """규칙마다 그 규칙이 막으려던 버그를 되살려 실패하는지 본다.

    다른 규칙까지 같이 실패하는 것은 상관없다 -- 확인할 것은 **해당 규칙이 잡는가**다."""
...
def test_rule_shapes_have_teeth() -> None:
    ...
    ② 정의부가 만족시키는 경우 -- require 가 `def 이름(` 에도 걸리면 호출을 지워도
       정의가 남아 통과다. 계산-강제가 그랬다.

    이 검사를 붙이자마자 **살아 있는 아무것도 걸러내지 못하는 규칙 둘을 더 잡았다**(RPM-분리의
    _is_rpm\(e\), 버전-보존의 _snapshot_code\(). 둘 다 손으로 쓴 red-green 은
    통과하고 있었다 -- 내 변형이 정의와 호출을 한꺼번에 바꿔서 우연히 실패했기
    때문이다. 실제 회귀(호출만 지움)에는 반응하지 못했다."""
    bad = lint.meta_defects()
    check(not bad, "규칙 모양에 약점이 있다: " +
          "; ".join(f"{r['id']}: {m[:80]}" for r, m in bad[:3]))
    print(f"    [규칙모양] 규칙 {len(lint.RULES)}개 -- OR 없음, 정의부에 안 걸림")
...
def test_rules_carry_evidence() -> None:
    ...
    하루 동안 실제로 겪은 버그이고, 그 자국을 잃으면 규칙만 남아 미신이 된다."""
    for r in lint.RULES:
...
def main() -> int:
    ...
    if FAILURES:
        ...
        for f in FAILURES:
            print("  -", f)
        return 1
    print(f"도구 규칙 검사: 정상이면 통과, 규칙 {len(lint.RULES)}개가 각자 실패한다, "
          "근거 커밋 보존 -- 통과")
    return 0
