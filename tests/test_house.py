...
try:
    ...
    with _tf3.TemporaryDirectory() as _d3:
        ...
        _답4 = C.run("!회사 전체", allow_write=True) or ""
        ok(_본3["인자"][:2] == ["--회로", "mera1_event_rec"],
           f"**`!회사 전체` 가 승인해 지은 그 회로를 넘긴다** ({_본3['인자']})")
        ok("mera1_event_rec" in _답4 and "마지막에 지은" in _답4,
           "**무엇을 도는지 답에 적는다** — 엉뚱한 회로를 돌고도 모르는 일이 없게")
        C.run("!회사 검증", allow_write=True)
        ok(_본3["인자"] == ["dv", "--회로", "mera1_event_rec", "--메일"],
           f"**한 사람을 불러도 같은 회로다** ({_본3['인자']}) — 한쪽만 새 회로를 "
           "돌면 두 보고서가 왜 다른지 알 수 없다")
        C.run("!회사 전체 fir", allow_write=True)
        ok(_본3["인자"][:2] == ["--회로", "fir"],
           f"이름을 대면 그것을 돈다 ({_본3['인자']})")
finally:
    _DES.등록부길 = _옛등록부
