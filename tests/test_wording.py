...
ok("이번 대목의 재료(제도·매체)" in _b2, "제도와 매체를 한 자리에 묶는다")
ok("편입 시험" in _b2 and "편지" in _b2, "둘 다 목록에는 그대로 있다")
ok("이번 대목의 제한" in _b2, "제한을 준다")
# "우연이 문제를 풀지 않기" 는 [리얼리즘] 과 [표류가 먼저다] 가 이미 말한다.
# 세 번째로 적으면 자리만 먹는다 -- 여기서는 **구체적이어야 한다**만 남긴다.
ok("못 하는 이유는 구체적이어야 한다" in _b2, "제한은 구체적이어야 한다고만 말한다")
...
ok(len(_b5) < 5200, f"그래도 {len(_b5)}자에 담긴다  ← 이름은 짧고 설명은 뽑힌 것에만")

print()
print("[안팎] **밖만 적으면 관찰 일지, 안만 적으면 일기다**")
_b6 = W.brief(["x" * 60], "씨", 4)
ok("안에서 새는 것(내현)" in _b6, "안쪽도 뽑는 자리가 있다")
ok("고백조로 쓰면 신파가 되고" in _b6, "무겁게 가지 말라고 한다")
ok("결론은 내지 마라" in _b6, "사유는 하되 결론은 안 낸다  ← 가볍고 위트 있게")
ok("'트라우마' 라고 부르지 마라" in " ".join(W.INNER.values()),
   "이름을 붙이면 진단서가 된다")
for _k in ("쓸데없는 내력", "몸의 욕구", "짐승 쪽", "따뜻한 데", "생각을 굴린다"):
    ok(_k in W.INNER, f"안쪽 재료: {_k}")
ok("부딪쳐라" in _b6, "주변과 부딪치는 자리가 있다")
ok("보기만 하면 이 사람은 카메라다" in _b6,
