...
_led2, _end2 = led_of(("이산화", {"식": "a = b", "정의역": r"F = \mathbb{C}", "치수": 91}))
_r2 = MO.chain(_led2, _end2)[0]
ok(_r2["칸"]["정의역등급"]["움직임"] == 0 and not _r2["칸"]["정의역등급"]["어긋남"],
   "안 움직인 것은 어긋남이 아니다 -- 연산자가 그 양을 안 건드린 것이다")
_led3, _end3 = led_of(("표수 이동", {"식": "a = b", "정의역": r"F = F_2", "치수": 91}),
                      ("이산화", {"식": "a = b", "정의역": r"F = \mathbb{R}", "치수": 91}))
_r3 = MO.chain(_led3, _end3)
ok(_r3[1]["칸"]["정의역등급"]["움직임"] == 1 and _r3[1]["칸"]["정의역등급"]["선언"] == "↓",
   "이산화라고 해 놓고 유한(1)에서 연속(3)으로 올라갔다")
ok(_r3[1]["칸"]["정의역등급"]["어긋남"], "**반대로 간 걸음에 어긋남이 선다**")
ok(len(_led3["spaces"]) == 3 and SP.get(_led3, _end3) is not None,
