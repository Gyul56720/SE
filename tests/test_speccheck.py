...
# ==================================================================== 제안서 표본
# **실측 2026-09-22 — 제안서 `mera_rec` (6쪽, 110.2초, 모델 씀).**
# 파이프라인이 끝까지 돌았다. 그런데 그 6쪽 안에 아래 것들이 그대로 들어 있었고
# **S001~S024 가 한 건도 안 물었다.** 그 문서를 여기 표본으로 박아 둔다.
print("\n[제안서 mera_rec 표본]")

_제안 = S.스펙(
    요청=("----- 첨부: 1551965127165608020_spec.md -----\n"
        "# MERA-1 v1.0 Event Recorder Core\n"
        "계측·시험장비용입니다. 목표 주파수 500 MHz, 공급 전압 0.8 V.\n"
        "junction 0 ~ 100 °C (Extended). functional safety 인증 없음.\n"
        "DDR 출력은 AXI4-MM master, address 64-bit, data 128-bit.\n"
        "제어는 AXI4-Lite. RFDC 입력은 AXI4-Stream canonical 256-bit.\n"
        "포트 이름은 AMD 규약을 따라 주세요 (`s_axis_*` · `m_axi_*` · `s_axi_*` · "
        "`*_aclk` · `*_aresetn`) — Vivado IP Integrator 가 묶는 근거가 그 이름입니다."),
    쓰임새=["계측/시험장비"], 문제=["속도", "면적", "정밀도", "신뢰성"],
    회로=["ADC", "AXI"], 수={"주파수_Hz": [5e8], "전압_V": [0.8], "온도_C": [100]},
    이름="mera_rec",
    포트=[
        {"이름": "m_axi_wdata", "방향": "output", "폭": 128, "뜻": "DDR 기록용"},
        {"이름": "s_axi_lite_awaddr", "방향": "input", "폭": 32, "뜻": "제어용 AXI4-Lite"},
        {"이름": "s_axis_rfdc_tdata", "방향": "input", "폭": 256, "뜻": "RFDC canonical"},
        {"이름": "s_axis_tvalid", "방향": "input", "폭": 1, "뜻": "데이터 유효 신호"},
        {"이름": None, "방향": "input", "폭": 1, "뜻": "외부 트리거 입력"},
    ],
    클럭=[{"이름": "aclk", "주기_ns": 2.0, "도메인": "RFDC"}],
    모드=[{"이름": "CIRCULAR", "전력": "최대", "무엇": "트리거 대기"},
        {"이름": "RECORDING", "전력": "최대", "무엇": "DDR 전송"}],
    목표=[{"항목": "주파수", "값": "500 MHz", "어떻게 잴 것인가": "STA setup margin"},
        {"항목": "결정성", "값": "Jitter Free",
         "어떻게 잴 것인가": "트리거 수신부터 기록 시작까지의 사이클 수 고정 검증"}],
    검증계획=[{"시나리오": "정상 트리거 캡처", "노리는 것": "정합성"}],
    위험=["ASIC 공정 내 SRAM 매크로 면적 초과"],
    모른다=["어디에 쓰는지 (온도 등급 · 안전 등급이 여기서 나온다)",
         "RFDC TDATA 구체적 폭 및 포맷 (Vivado 생성 결과 대기)",
         "다채널 확장을 위한 고정 오프셋 레지스터의 구체적 비트 폭",
         "ASIC 공정 노드 정보에 따른 정확한 SRAM/FF 면적 데이터",
         "RFDC TDATA 폭(TBD)",
         "I/Q 비트 정렬 방식(TBD)",
         "SRAM vs FF 면적 비교 데이터"],
)

_난 = C.검사(_제안)
_규칙 = {x["규칙"] for x in _난}


def _문(규칙):
    return [x for x in _난 if x["규칙"] == 규칙]


# --- S025: `s_axis_rfdc_tdata` 와 `s_axis_tvalid` 는 Vivado 가 두 묶음으로 본다
_25 = _문("S025")
ok(any(x["자리"] == "포트 s_axis_tvalid" for x in _25),
   "**S025 접두사가 갈라진 스트림 포트를 잡는다** (`s_axis_tvalid` 가 "
   "`s_axis_rfdc_*` 에서 떨어져 나왔다)")
ok(all(x["고친값"].get("이름") == "s_axis_rfdc_tvalid"
       for x in _25 if x["자리"] == "포트 s_axis_tvalid"),
   "고칠 이름까지 댄다 — `s_axis_rfdc_tvalid`")

# 진짜로 스트림이 둘이면 안 문다 (각자 제 tdata 를 갖는다)
_둘 = S.스펙(포트=[{"이름": "s_axis_a_tdata", "방향": "input", "폭": 32, "뜻": ""},
              {"이름": "s_axis_a_tvalid", "방향": "input", "폭": 1, "뜻": ""},
              {"이름": "s_axis_b_tdata", "방향": "input", "폭": 32, "뜻": ""},
              {"이름": "s_axis_b_tvalid", "방향": "input", "폭": 1, "뜻": ""}])
ok(not [x for x in C.검사(_둘) if x["규칙"] == "S025"],
   "**스트림이 진짜 둘이면 안 문다** — 거짓 거절을 막는다")

# --- S026: 이름이 `None` 인 포트
ok(_문("S026"), "**S026 이름 없는 포트를 잡는다** (`None` 이 글자로 찍혔다)")

# --- S027: 이미 읽은 칸을 '모른다' 에 올렸다
_27 = _문("S027")
ok(any("어디에 쓰는지" in x["자리"] for x in _27),
   "**S027 쓰임새를 읽어 놓고 '어디에 쓰는지' 를 모른다고 한 것을 잡는다**")

# --- S028: 같은 것을 여러 줄로
_28뺌 = [x for x in _문("S028") if x["갈래"] == "고침"]
ok(any("RFDC TDATA 폭(TBD)" in x["자리"] for x in _28뺌),
   "**S028 겹친 '모르는 것' 을 잡는다** (`RFDC TDATA 폭(TBD)`)")

# --- S029 / S030
ok(_문("S029"), "**S029 모드 전력이 전부 같으면 잡는다** (CIRCULAR·RECORDING 둘 다 '최대')")
_30 = _문("S030")
ok(_30 and "결정성" in _30[0]["자리"],
   "**S030 `Jitter Free` 는 목표가 아니라 광고다**")
ok(bool(_30) and "사이클 수 고정" in (_30[0]["말"] or ""),
   "재는 법 칸에 적힌 진짜 목표를 같이 가리킨다 — 막기만 하면 사람은 뭘 쓸지 모른다")

# --- 고치고 나면 '모르는 것' 수가 실제로 준다 (요약 줄에 실리는 수다)
_전 = len(_제안.모른다)
_고친스펙, _고친목록 = C.고치기(_제안, _난)
_후 = len(_고친스펙.모른다)
ok(_후 < _전, f"**'모르는 것' 수가 실제로 줄어든다** ({_전} → {_후}) — "
   "이 수가 요약 줄의 '아직 모르는 것 N개' 다")
ok(not any("어디에 쓰는지" in x for x in _고친스펙.모른다),
   "읽힌 칸은 목록에서 빠졌다")
ok(any("모르는 것에서" in x for x in _고친목록),
   "**무엇을 뺐는지 적는다** — 조용히 지나가지 않는다")
ok(any(p.get("이름") == "s_axis_rfdc_tvalid" for p in _고친스펙.포트),
   "접두사도 실제로 고쳐진다")

# --- 내 코드가 지어낸 수: `_왜필요` 의 '자동차 −40~125 °C'
_왜 = A._왜필요("어디에 쓰는지 (온도 등급 · 안전 등급이 여기서 나온다)")
ok("125" not in _왜 and "자동차" not in _왜,
   f"**`_왜필요` 가 온도 범위를 지어내지 않는다** ({_왜[:40]!r}) — "
   "요청에는 junction 0~100 °C 라고 적혀 있었다. 모델이 아니라 내 코드가 지어냈다")

# --- 첨부 구분선이 제목·요약으로 새지 않는다
_벗 = S.요청다듬기("----- 첨부: 1551965127165608020_spec.md -----\n# MERA-1 v1.0")
ok("첨부:" not in _벗 and "MERA-1 v1.0" in _벗,
   f"**첨부 구분선만 벗기고 내용은 안 건드린다** ({_벗!r})")
ok(S.읽기("----- 첨부: a.md -----\n500 MHz FIR 필터").요청.startswith("500 MHz"),
   "`읽기()` 가 벗긴 글을 스펙의 `요청` 으로 삼는다 — 표지가 여기서 뽑힌다")



# ============================================ 남은 넷 (사용자: "남은 넷도 다 잡아줘")
print("\n[남은 넷]")

# --- 1. 요청이 수까지 못박은 포트가 통째로 없다 (S031)
_31 = [x for x in _난 if x["규칙"] == "S031"]
ok(any("m_axi" in x["자리"] for x in _31),
   "**S031 `m_axi_awaddr` 가 없는 것을 잡는다** — 요청은 address 64-bit 를 "
   "못박았는데 그 수를 놓을 포트가 없었다")
ok(any("한 칸만 빠졌다" in x["말"] for x in _31),
   "S024('통째로 없다')와 다른 것임을 말한다")
_있 = S.스펙(요청="DDR 출력은 AXI4-MM master, address 64-bit, data 128-bit 입니다.",
          포트=[{"이름": "m_axi_awaddr", "방향": "output", "폭": 64, "뜻": ""},
              {"이름": "m_axi_wdata", "방향": "output", "폭": 128, "뜻": ""}])
ok(not [x for x in C.검사(_있) if x["규칙"] == "S031"],
   "**다 있으면 안 문다** — 거짓 거절을 막는다")

# --- 2. 요청이 글자로 못박은 접두사 (S032)
_32 = [x for x in _난 if x["규칙"] == "S032"]
ok(any(x["자리"] == "포트 s_axi_lite_awaddr" for x in _32),
   "**S032 `s_axi_lite_awaddr` 를 잡는다** — 요청은 `s_axi_*` 라고 적었다")
ok(any(x["고친값"].get("이름") == "s_axi_awaddr" for x in _32),
   "고칠 이름을 댄다 — `s_axi_awaddr`")
_안적음 = S.스펙(요청="AXI4-Lite 로 제어합니다.",
            포트=[{"이름": "s_axi_lite_awaddr", "방향": "input", "폭": 32, "뜻": ""}])
ok(not [x for x in C.검사(_안적음) if x["규칙"] == "S032"],
   "**요청이 접두사를 안 적었으면 안 문다** — 우리 취향을 강요하지 않는다")

# --- 3. 요청 표에 못박힌 수가 판독으로 들어온다
_스펙본 = ("| 항목 | 값 |\n|---|---|\n| PRE | **4096** complex sample |\n"
        "| POST | **16384** complex sample |\n| payload | **80 KiB** |\n"
        "| header | **64 byte 고정** |\n"
        "| DDR 출력 | **AXI4-MM master**, address **64-bit**, data **128-bit** |\n"
        "| RFDC TDATA 폭 | **TBD** — Vivado 생성 결과를 봐야 합니다 |\n")
_뽑 = S.표에서읽기(_스펙본)
_항목 = {a for a, _ in _뽑}
ok({"PRE", "POST", "payload", "header"} <= _항목,
   f"**요청 표의 수를 그대로 들고 온다** ({sorted(_항목)})")
ok(all("4096" in v for a, v in _뽑 if a == "PRE"),
   "값을 풀지도 고치지도 않는다 — 적힌 그대로")
ok("RFDC TDATA 폭" not in _항목,
   "**수가 없는 줄(TBD)은 안 들고 온다** — 그건 '모르는 것' 이지 '못박은 것' 이 아니다")
ok(not any(set(a) <= set("-: ") for a, _ in _뽑), "표의 구분선은 안 들어온다")
ok(S.읽기(_스펙본).못박힌수, "`읽기()` 가 이것을 스펙에 담는다 — 제안서가 여기서 뽑는다")

# --- 4. 그림 글자가 상자를 안 넘는다
from house import viz as V  # noqa: E402

ok(V.군말털기("스펙 판독\\n(코드)") == "스펙 판독 (코드)",
   "**`\\n` 두 글자를 턴다** — SVG 는 그것을 줄바꿈으로 안 읽는다. 제안서에 "
   "`판독\\n(코드` 가 그대로 찍혀 있었다")
ok(V.군말털기("**이 제안서**") == "이 제안서",
   "**markdown 별표를 턴다** — SVG 에 markdown 은 없다")

# 실제 제안서가 쓰는 그 단계들로 앉혀 보고 **폭을 잰다**
_단계 = ["요청 (자연어)", "스펙 판독\\n(코드, 모델 없이)", "스펙 보강\\n(모델)",
       "선행조사", "**이 제안서**", "사람 승인", "RTL·TB 생성", "관문 7개", "5명 실행"]
_칸폭 = max(70, (680 - 28 - 22 * 8) / 9) - 8
_넘친것 = []
for _t in _단계:
    _줄, _크 = V.앉히기(_t, 10.5, _칸폭, 2)
    for _l in _줄:
        if V.글자너비(_l, _크) > _칸폭 + 0.5:
            _넘친것.append((_l, round(V.글자너비(_l, _크), 1)))
ok(not _넘친것,
   f"**흐름도의 아홉 칸이 전부 상자 안에 들어간다** (칸 {_칸폭:.0f}px, "
   f"넘친 것 {_넘친것})")

_부제 = "RFDC의 가변 TDATA를 256-bit 내부 canonical 포맷으로 변환"
_상자 = 640 / 4 - 18
_줄2, _크2 = V.앉히기(_부제, 8.5, _상자 - 8, 2)
ok(_줄2 and all(V.글자너비(x, _크2) <= _상자 - 8 + 0.5 for x in _줄2),
   f"**블록도 부제가 상자 안에 들어간다** (상자 {_상자:.0f}px → {_줄2})")
ok(len(_줄2) >= 2, "긴 부제는 접어서 넣는다 — 잘라 버리지 않는다")

# 잘라야만 들어가는 경우에는 **잘랐다고 보인다**
_줄3, _크3 = V.앉히기("가" * 200, 8.5, 60, 1)
ok(_줄3 and _줄3[0].endswith("…"), "접어도 줄여도 안 되면 자르고 `…` 를 남긴다")
ok(V.글자너비(_줄3[0], _크3) <= 60.5, "자른 것도 상자 안이다")
...
if fails:
    ...
    raise SystemExit(1)
print("speccheck: 네 결함 · 가름 · 고치기 · 안 건드림 · 되묻기 · "
      "제안서 표본(S025~S030) -- 통과")
