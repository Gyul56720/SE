...
print("\n[자리] **고치라는 말이 맨 앞인가**")
# 실측 2026-09-09: 사용자가 시킨 것(액션 다섯 박 · 카메라워크 · 직함 · 부상 기전 ·
# 개그 · 따옴표 · 리듬 · 외모…) 열여섯 가지가 **전부 프롬프트에 실려 있었는데도**
# 원고가 한 줄도 안 지켰다. 자리를 재 보니 "여기만 고쳐라" 가 8,455자 중 8,238자
# 자리 -- **97% 지점**이었고, 그 앞에 지시 줄 여든 개와 굵은 글씨 여든여덟 군데가
# 있었다. 전부가 제일 중요하다고 하면 아무것도 제일 중요하지 않다.
_bb = audit.sample("lanobe", 0.6, chunks=1)
_bb["chunks"] = ["그는 걸었다. " * 60 + "눈이 왔다. " * 60]     # 어긋나게 만든다
_pp = flow.write_prompt(_bb)
_kk = "[직전 덩어리에서 어긋난 것]"
ok(_kk in _pp, "어긋난 덩어리에는 고치라는 말이 실린다")
ok(_pp.index(_kk) < len(_pp) * 0.15,
   f"그 말이 맨 앞에 있다 ({_pp.index(_kk) / len(_pp):.0%} 지점)  ← 끝에 두면 묻힌다")
ok(_pp.count(_kk) == 1, "한 번만 나온다  ← 같은 말을 두 번 하면 프롬프트만 길어진다")
_clean = audit.sample("lanobe", 0.6, chunks=0)
ok(flow.write_prompt(_clean).count(_kk) <= 1, "어긋난 것이 없으면 그 자리는 빈다")
