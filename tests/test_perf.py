...
print("\n== 출력대조: 같은 입력에 같은 출력인가 (Φ' ⇒ Φ 의 유한 확인) ==")
ok(PF.출력대조({"바탕": ["a", "a"], "후보": ["a", "a"]})["출력대조"] == "같음",
   "둘 다 한결같고 서로 같으면 **같음** -- 그 입력·그 관측에서 의미가 보존됐다")
_다 = PF.출력대조({"바탕": ["a", "a"], "후보": ["b", "b"]})
ok(_다["출력대조"] == "다름" and "의미가 바뀌었다" in _다["출력말"],
   f"지문이 다르면 **다름** -- 더 빠른 프로그램이 아니라 다른 프로그램이다")
ok(PF.출력대조({"바탕": ["a", "z"], "후보": ["a", "a"]})["출력대조"] == "못잼",
   "**한쪽 안에서 갈리면 못잼** -- 비결정적 워크로드로는 의미를 대조할 수 없다(0 으로 안 채운다)")
ok(PF.출력대조({"바탕": [], "후보": []})["출력대조"] == "못잼", "출력을 못 모았으면 못잼")
ok(PF.지문("같다") == PF.지문("같다") and PF.지문("가") != PF.지문("나"), "지문이 글을 가른다")

print("\n== 관측은 칸이다: 붙여 세면 칸 사이를 옮기는 변화가 안 보인다 ==")
# **실측 2026-09-13.** `한번` 이 `stdout + stderr` 를 붙여서 지문을 냈다. 그래서 출력 전부를
# stdout 에서 stderr 로 옮긴 프로그램의 지문이 바탕과 **글자 하나까지 같았고**(6c6915b81725)
# 판정은 `같음` 이었다. 로그 리다이렉트는 흔한 "최적화" 이고, stdout 을 파이프로 받는 쪽에는
# 그것이 출력이 사라진 것이다. 관측을 하나로 보면 이 한 칸이 통째로 안 보인다.
_나 = {"나간": "scan done: 3 findings\n", "샌": "", "끝값": 0}
_샘 = {"나간": "", "샌": "scan done: 3 findings\n", "끝값": 0}
ok(PF.지문(_나["나간"] + _나["샌"]) == PF.지문(_샘["나간"] + _샘["샌"]),
   "붙여 세면 stdout->stderr 이동이 **같은 지문**이 된다 (이것이 있던 구멍이다)")
ok(PF.관측지문(_나) != PF.관측지문(_샘),
   f"**칸으로 세면 갈린다** ({PF.관측지문(_나)} vs {PF.관측지문(_샘)})")
ok(PF.출력대조({"바탕": [PF.관측지문(_나)] * 3, "후보": [PF.관측지문(_샘)] * 3})["출력대조"] == "다름",
   "**출력을 stderr 로 옮긴 후보는 REJECT** -- 더 빠른 프로그램이 아니라 다른 프로그램이다")
ok(PF.관측지문(_나) == PF.관측지문(dict(_나)),
   "같은 관측은 같은 지문이다 -- 칸을 나눈 값으로 거짓빨강을 내지 않는다")
ok(PF.관측지문({"나간": "ab", "샌": ""}) != PF.관측지문({"나간": "a", "샌": "b"}),
   "**길이를 앞에 붙여 경계를 못 넘게 한다** -- 안 그러면 `ab|` 와 `a|b` 가 같아진다")
ok("끝값" not in PF.관측칸 and "시간" not in PF.관측칸,
   f"관측칸에 시간·자원·끝값이 **없다** ({PF.관측칸}) -- 끝값은 터짐이 더 세게 막고, "
   "시간은 J 다(O 에 넣으면 빨라진 후보가 전부 `다름` 이 되어 아무것도 못 받는다)")
_맛 = PF.관측맛보기(_샘)
ok("[나간]" in _맛 and "[샌]" in _맛 and "3 findings" in _맛, f"맛보기가 **어느 칸인지** 보여 준다")
...
try:
    ...
    ok(any("빠름" in x for x in r["까닭"]), f"몇 % 빨라졌는지 적는다 ({[x for x in r['까닭'] if '빠름' in x][0][:60]})")
    ok(r["잰것"].get("출력대조") == "같음",
       f"**빠른 후보의 출력이 바탕과 같다** -- 성능을 재는 그 실행으로 같이 봤다 ({r['잰것'].get('출력대조')})")
    ok(any("출력대조" in x for x in r["까닭"]), "ACCEPT 까닭에 출력대조를 적는다")
    ...
    print("\n== 출력이 다르면 REJECT -- 빨라도 다른 프로그램이다 ==")
    # 같은 시간을 쓰면서 **다른 것을 찍는** 후보. 성능은 같으니 시간으로는 안 걸린다.
    다른출력 = 판짓기(0.05)
    (다른출력 / "일.py").write_text("import time\ntime.sleep(0.05)\nprint('다른 답')\n",
                                encoding="utf-8")
    git(다른출력, "add", "-A"); git(다른출력, "commit", "-qm", "출력을 바꿨다")
    try:
        r다 = PF.성능개선(느림, 다른출력, "시늉", 반복=5, V=초록V, 말하기=lambda s: None)
        ok(r다["결정"] == "REJECT" and any("의미가 바뀌었다" in x for x in r다["까닭"]),
           f"**0.30 -> 0.05초로 빨라도 출력이 다르면 REJECT** ({r다['까닭'][:1]})")
        ok(r다["잰것"].get("출력대조") == "다름", f"출력대조가 다름이다 ({r다['잰것'].get('출력대조')})")
    finally:
        shutil.rmtree(다른출력, ignore_errors=True)
