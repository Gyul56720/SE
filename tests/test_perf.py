...
try:
    ...
    # **앞을 자르면 안 된다.** 실측 2026-09-13: `instance-20260826-2010` 이 `instance-` 로 잘려
    # `instance-/2` 로 찍혔다 -- 클라우드 인스턴스는 앞이 다 같아서 다른 기계가 같아 보인다.
    긴 = {"id": "x", "때": "2026-01-01T00:00:00Z", "결정": "재기",
         "기계": {"이름": "instance-20260826-2010", "코어": 2}}
    PF.적기(집, {"결정": "재기", "Δ": None, "잰것": {"워크로드": "관문", "바탕중앙값": 10.06}}, "", "")
    줄들 = [x for x in (집 / PF.성능경로).read_text(encoding="utf-8").splitlines() if x.strip()]
    import json as _j
    마 = _j.loads(줄들[-1]); 마["기계"] = 긴["기계"]
    줄들[-1] = _j.dumps(마, ensure_ascii=False)
    (집 / PF.성능경로).write_text("\n".join(줄들) + "\n", encoding="utf-8")
    글기계 = PF.보고(집)
    ok("0826-2010" in 글기계,
       f"**이름의 구별되는 뒤쪽을 남긴다** -- `instance-/2` 로는 두 인스턴스가 같아 보인다")
