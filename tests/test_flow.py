...
def clean(tag: int) -> str:
    return "\n".join(
        # 점층도 섞는다 -- 앞 문장을 받아 올리는 문장이 없으면 리듬 자에 걸려 재시도가
        # 늘고, 여기서 세는 횟수가 어긋난다.
        [(f"{tag}년 {i}월의 항구는 오후 세 시부터 어두워졌고, 바람에는 생선과 디젤과 "
          f"눈 냄새가 한꺼번에 섞여 있었다."
          if i % 3 else
          f"아니, 냄새라기보다는 {tag}년의 그 겨울이 통째로 실려 온 것에 가까웠다.")
         for i in range(1, 13)]
        + [f'"{w} 드실래요?"' for w in ("커피", "차", "물", "맥주")]
        # 긴 대사가 **둘** 있어야 한다. 하나만 두었더니 확산 자에 걸렸다 -- 개수 하한만
        # 있으면 모델이 최소로 맞춰 대사가 죄다 짧아진다는 실측 뒤에 하한을 올렸다.
        + [f'"아뇨, 괜찮습니다. 방금 마셨거든요. 아니, 마신 것 같기도 하고 아닌 것 '
           f'같기도 하고, {tag}년쯤부터는 그게 잘 구분이 안 갑니다."',
           f'"구분이 안 가면 그냥 드시면 되잖아요. 나는 그런 걸로 고민해 본 적이 없는데, '
           f'하긴 나는 고민이라는 걸 잘 안 하는 편이라 그게 자랑은 아니겠습니다만."']
