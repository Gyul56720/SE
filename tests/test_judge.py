...
try:
    ...
    print("\n== 분모: 덜 재서 거짓초록수가 준 것을 개선이라 하지 않는다 ==")
    # 사용자(2026-09-13): "D_0 가 100개를 재고 D_1 이 20개를 쟀다면 점수 0.90 -> 0.95 는
    # 아무것도 증명하지 않는다." `거짓초록수` 는 날 셈이고 무게가 -3 이라 **덜 재면 J 가 오른다.**
    ...
    옛ΔJ = Jd.ΔJ
    가짜 = {"앞": {"관찰파일수": 0, "거짓초록수": 100, "미정의수": 0, "검사없는파일수": 0, "잰변형수": 100},
          "뒤": {"관찰파일수": 0, "거짓초록수": 10, "미정의수": 0, "검사없는파일수": 0, "잰변형수": 20}}
    전Y = sum(Jd.무게[k] * 가짜["앞"].get(k, 0) for k in Jd.무게)
    후Y = sum(Jd.무게[k] * 가짜["뒤"].get(k, 0) for k in Jd.무게)
    ok(후Y > 전Y, f"**점수만 보면 올랐다** ({전Y:+.1f} -> {후Y:+.1f}) -- 거짓초록 90개가 사라졌으니까")
