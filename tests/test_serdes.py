...
print("\n[판정 색인 ROM -- 곱셈기 0개로 DFE 를 일반화한다]")
# 선형 DFE 는 과거 판정의 **선형** 함수로 뺄 값을 정한다. 같은 입력에 임의 함수를
# 담으면 2^깊이 칸 표 하나이고 곱셈기는 여전히 0개다 -- **엄밀한 일반화**다.
#
# ## 대조가 먼저다 -- 선형 채널에서 이기면 새는 것이다
# 선형 채널에서는 최적 후행커서 제거가 선형이므로 ROM 이 DFE 를 못 이겨야 한다.
# 실측 2026-09-16 (25dB · SNR30 · 30만 비트 · 씨 8개):
#     선형 DFE 6.5595e-04 · ROM8 6.5060e-04 -> 1.01배, ROM 이 이긴 씨 3/8,
#     가르려면 3억 6천만 비트. **비긴다 = 안 샌다.**
민 = dict(비트수=60000, 손실dB=25.0, SNRdB=30.0, sps=8, FFE탭=11, DFE탭=8,
        ADC비트=7, ADC풀스케일시그마=2.5, 압축=0.0, 씨=4)
선형DFE = serdes.링크(**민)
롬 = serdes.링크(ROM깊이=8, **민)
ok(롬["ROM칸수"] == 256, f"깊이 8 이면 칸이 256 개다 ({롬['ROM칸수']})")
가를것 = serdes.가르려면몇비트(선형DFE["BER"], 롬["BER"]) if 롬["BER"] > 0 else float("inf")
ok(가를것 > 롬["잰비트"],
   f"**선형 채널에서는 ROM 이 DFE 를 못 이긴다** "
   f"({선형DFE['오류수']} 대 {롬['오류수']}, 가르려면 {가를것:,.0f} 비트 > 잰 {롬['잰비트']:,})")

# ## 값을 하는 자리 -- 압축 뒤에 필터가 있어 손상이 **판정에 달린 비선형**일 때
# 실측(압축뒤대역 0.35 · 씨 8개, 선형 FFE11+DFE8 대비):
#     표(메모리없음)+선형  1.91배      ROM8  **3.08배**      MLP 곱셈96  5.80배
#     표+ROM8  2.11배 <- **서로 방해한다**(7/8 씨에서 나빠진다)
#     표+MLP96 **21.30배** <- 이것이 이긴다. ROM 은 0/8 로 진다(0.10배)
# 그러니 **ROM 은 공짜 이득이지 망의 대체물이 아니다.**
좁 = dict(비트수=60000, 손실dB=25.0, SNRdB=30.0, sps=8, FFE탭=11, DFE탭=8,
        ADC비트=7, ADC풀스케일시그마=2.5, 압축=1.0, 압축뒤대역=0.35, 씨=4)
민좁 = serdes.링크(**좁)
롬좁 = serdes.링크(ROM깊이=8, **좁)
ok(롬좁["오류수"] < 민좁["오류수"],
   f"**창 안에서는 ROM 이 DFE 를 이긴다** (오류 {민좁['오류수']} -> {롬좁['오류수']}) "
   "-- 곱셈기를 하나도 안 더 쓰고")

# ## 깊이는 면적이 아니라 **학습 데이터**가 제한한다
# 실측: 깊이 4 -> 2.32배 · 6 -> 2.62 · 8 -> 3.08 · 10 -> 3.09 · **12 -> 2.13(과적합)**
# 4096 칸에 학습 심볼이 9만이면 칸당 22개다. 표는 칸마다 독립 파라미터라 데이터가
# 2^깊이 에 비례해 필요하다 -- 망이 이기는 까닭이 여기다(파라미터 공유).
깊12 = serdes.링크(ROM깊이=12, **좁)
ok(깊12["ROM칸수"] == 4096, "깊이 12 면 4096 칸")
ok(깊12["오류수"] >= 롬좁["오류수"],
   f"**깊이를 12 로 늘리면 나아지지 않는다** ({롬좁['오류수']} -> {깊12['오류수']}) "
   "-- 칸이 늘수록 칸당 학습 표본이 줄어 잡음을 외운다")

# 표가 모자란 칸은 배우지 않고 선형 DFE 로 되돌린다 -- 안 그러면 드문 패턴에서 터진다
적은것 = serdes.링크(ROM깊이=12, ROM최소표본=10**9, **좁)
ok(적은것["ROM되돌린칸"] == 4096,
   f"**최소표본을 못 채우면 전부 되돌린다** ({적은것['ROM되돌린칸']}/4096)")

# ## **표가 학습 구간 밖을 보면 안 된다** -- 이 누수는 위 검사들이 다 놓쳤다(실측)
# `쓸것 = min(끝, ...)` 에서 `끝` 을 빼도 위 항목이 전부 통과했다. 누수가 표의
# 추정을 좋게 만들 뿐 b_t 를 외우지는 않아서 티가 안 난다.
# **학습 구간을 좁히면 드러난다**: 정직하게 배우면 칸이 표본을 못 채워 되돌려지는데,
# 평가 구간까지 보면 채워진다. 되돌린 칸 수가 그대로 탐침이다.
쪼금 = dict(좁); 쪼금["학습비율"] = 0.02
깊12쪼금 = serdes.링크(ROM깊이=12, **쪼금)
ok(깊12쪼금["ROM되돌린칸"] == 4096,
   f"**학습 2%면 깊이 12 의 칸을 하나도 못 채운다** ({깊12쪼금['ROM되돌린칸']}/4096) "
   "-- 이보다 적으면 표가 평가 구간을 본 것이다")
롬쪼금 = serdes.링크(ROM깊이=8, **쪼금)
ok(롬쪼금["오류수"] / max(롬쪼금["잰비트"], 1) >
   2.0 * 롬좁["오류수"] / max(롬좁["잰비트"], 1),
   f"**학습을 줄이면 ROM 이 나빠진다** (학습 30%: {롬좁['오류수']}/{롬좁['잰비트']:,} -> "
   f"학습 2%: {롬쪼금['오류수']}/{롬쪼금['잰비트']:,}) -- 안 나빠지면 어디선가 다 보고 있다")

print("\n[표본 색인 표 -- 판정 영역의 천장을 넘는다, 곱셈기 0개로]")
# 압축 뒤 필터가 섞는 것은 **아날로그 값**인데 DFE·판정ROM·판정볼테라는 ±1 판정만
# 본다(진폭을 이미 버렸다). 그래서 셋이 같은 천장에 걸렸다 -- 실측 2026-09-16,
# 압축뒤대역 0.35 · 30만 비트 · 씨 8개, 선형 FFE11+DFE8 대비:
#
#     판정볼테라 차수2 (79~137 파라미터)    1.04~1.10배
#     판정볼테라 차수3 (299~1351 파라미터)  2.68~2.91배
#     판정ROM 꽉 (256~1024 파라미터)       **3.08~3.09배**   <- 천장
#     MLP (곱셈 96개)                      5.80배
#     **표본색인표 창2·4비트 (256칸)**      **7.49배** [4.13, 12.21]  6/6
#
# 파라미터를 1,351개까지 늘려도 2.91배에서 멈춘다 -- **파라미터 수가 아니라 입력이
# 문제였다.** 표가 담는 것은 판정 한 비트이므로 256비트 ROM 하나다.
창 = dict(비트수=100000, 손실dB=25.0, SNRdB=30.0, sps=8, FFE탭=11,
        압축=1.0, ADC비트=7, ADC풀스케일시그마=2.5, 압축뒤대역=0.35, 씨=4)
선형창 = serdes.링크(DFE탭=8, **창)
표창 = serdes.링크(DFE탭=0, 표본표창=2, 표본표비트=4, **창)
ok(표창["ROM파라미터수"] == 256, f"창 2 · 4비트면 256칸 ({표창['ROM파라미터수']})")
ok(표창["오류수"] < 선형창["오류수"] / 2,
   f"**창 안에서 표본색인표가 선형을 2배 넘게 이긴다** "
   f"({선형창['오류수']} -> {표창['오류수']}) -- 곱셈기 0개로")
롬창 = serdes.링크(DFE탭=8, ROM깊이=8, **창)
ok(표창["오류수"] < 롬창["오류수"],
   f"**판정 영역 천장(판정ROM)을 넘는다** (ROM8 {롬창['오류수']} -> "
   f"표본표 {표창['오류수']})")

# ## 대조 -- 선형 채널에서는 **져야** 한다
# 표본 두 개만 보므로 FFE11+DFE8 의 스팬을 못 당한다. 실측: 압축 0 · 필터 0.35 에서
# **0.27배** [0.16, 0.40], 0/6 씨. 여기서 이기면 새는 것이다.
민창 = dict(창); 민창["압축"] = 0.0
선형민 = serdes.링크(DFE탭=8, **민창)
표민 = serdes.링크(DFE탭=0, 표본표창=2, 표본표비트=4, **민창)
ok(표민["오류수"] > 선형민["오류수"],
   f"**선형 채널에서는 표본색인표가 진다** (선형 {선형민['오류수']} vs "
   f"표 {표민['오류수']}) -- 최적이 선형인 자리에서 이기면 새는 것이다")

# 칸이 늘면 안 차서 무너진다 -- 천장이 데이터라는 것
넓은표 = serdes.링크(DFE탭=0, 표본표창=5, 표본표비트=3, **창)
ok(넓은표["표본표찬칸"] < 넓은표["ROM파라미터수"] // 4,
   f"**창 5 · 3비트는 칸이 거의 안 찬다** ({넓은표['표본표찬칸']}/"
   f"{넓은표['ROM파라미터수']}) -- 표의 천장은 면적이 아니라 학습 데이터다")
ok(넓은표["오류수"] > 표창["오류수"],
   f"그래서 넓은 표가 더 나쁘다 ({표창['오류수']} -> {넓은표['오류수']})")

# **표가 학습 구간 밖을 보면 안 된다** -- 판정ROM 때와 같은 부류이고, 여기서도
# 위 검사들이 전부 놓쳤다(실측). 같은 탐침을 쓴다: 학습을 2% 로 좁히면 정직하게는
# 칸이 거의 안 차는데, 평가 구간까지 보면 찬다.
쪼금창 = dict(창); 쪼금창["학습비율"] = 0.02
쪼금표 = serdes.링크(DFE탭=0, 표본표창=3, 표본표비트=4, **쪼금창)
찬것 = serdes.링크(DFE탭=0, 표본표창=3, 표본표비트=4, **창)
ok(쪼금표["표본표찬칸"] < 찬것["표본표찬칸"] / 3,
   f"**학습을 2%로 줄이면 찬 칸이 확 준다** ({찬것['표본표찬칸']} -> "
   f"{쪼금표['표본표찬칸']}/{쪼금표['ROM파라미터수']}) -- 안 줄면 표가 평가 구간을 본 것이다")

print("\n[판정 볼테라 -- 차수를 잘라도 판정 천장을 못 넘는다]")
# ±1 변수 위의 단항식은 곱도 ±1 이라 **볼테라 전개는 통째로 곱셈기가 없다.**
# 차수를 자르면 파라미터가 2^깊이 -> O(깊이^차수) 로 준다. 그래도 못 넘었다.
볼 = serdes.링크(DFE탭=8, ROM깊이=12, ROM차수=3, **창)
ok(볼["ROM파라미터수"] == 299,
   f"깊이 12 · 차수 3 이면 파라미터 299개 ({볼['ROM파라미터수']})")
ok(볼["ROM파라미터수"] < 1 << 12,
   "**꽉 찬 표(4096)보다 훨씬 적다** -- 이것이 파라미터 공유다")
