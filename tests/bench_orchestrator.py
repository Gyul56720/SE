...
두 세트가 있다. 축이 다르다.
  ...
  [b*] 1차: 계산이 어려운 문제. 2026-09-01 실측 결과 6/6 전승, 수리 0회 -- 실패했다.
       Tonelli-Shanks, meet-in-the-middle, 백트래킹, Edmonds-Karp, Decimal 뉴턴은 전부
       교과서 알고리즘이고 그것을 40줄 쓰는 건 LLM 이 가장 잘하는 일이다. 계산량이 큰 것과
       LLM 이 틀리는 것은 다른 축인데 계산량 축만 올렸다. 피드백 루프는 한 번도 발동하지
       않았고, 따라서 이 세트는 우리가 만든 것을 시험하지 못했다. 기준선으로 남겨둔다.

  [h*] 2차: '첫 시도가 틀리도록' 설계한 문제. 노리는 실패 축이 각각 다르다.
       - 예산 초과를 원리적으로 강제한다(순진한 방법이 확실히 60초를 넘게).
       - verifier 가 독립 검증을 할 수 없는 문제를 넣는다. 세는 문제는 검증도 결국 세는
         것이라 verifier 가 solve 를 그대로 베끼게 되고, solve 가 틀리면 verifier 도 같이
         틀린다 -> 거짓 양성이 나올 수 있는 유일한 구조다. 이 아키텍처의 진짜 위험이다.
       - 명세를 정확히 읽어야만 맞는 문제를 넣는다. 오해하면 solve 와 check 가 같은 오해를
         공유해 역시 거짓 양성이 된다.
       - 중간 산출물을 명시적으로 요구해 DAG 분해를 유도하고, 그 중간값이 정수 키 dict 라
         JSON 왕복 함정을 다시 지나게 한다.
...
실행:
    cd ~/SE
    set -a; . ./.env; set +a
    python3 tests/bench_orchestrator.py --only h1,h2,h3,h4,h5   # 2차 세트만 (권장)
...
# ---- 2차 세트: '첫 시도가 틀리도록' 설계한 문제들 ----------------------------
H1_ANSWER = 815449418          # F(10^10) mod (10^9+7). fast doubling 으로 확인 + 점화식 교차검증

H2_EDGES = [(0, 4), (0, 6), (0, 7), (0, 11), (1, 3), (1, 4), (1, 5), (1, 8), (1, 11),
            (2, 3), (2, 5), (2, 11), (3, 7), (3, 8), (3, 9), (3, 11), (4, 5), (4, 7),
            (4, 8), (5, 6), (5, 8), (5, 11), (6, 7), (6, 8), (7, 8), (7, 9), (7, 10),
            (8, 9), (8, 10), (9, 11)]
H2_ANSWER = 9058               # 비트마스크 DP 로 정확히 셈 (방향 구분: 역순도 별개)

H3_PROGRAM = """0: SET r0 200
1: SET r1 1
2: MUL r1 r0
3: DEC r0
4: JNZ r0 -2
...
8: OUT r0
9: HALT"""
H3_OUT = [2, 250]              # 정확히 구현했을 때의 출력
H3_REGS = [250, 2, 0, 0]
# 오답 시나리오(참고): 점프를 '다음 명령 기준'으로 오해 -> [51, 250]
#                      DEC 를 0에서 멈추게 오해 -> [0, 0]

H4_ANSWER = ("294691971451113867673068249642969330052841741033727797238849427."
             "2725311012294892450789977024999428651832")   # 2^1000 / 3^500, 소수점 40자리

H5_NUMS = [22874621, 184093, 234, 872053, 6417, 4848591,
           1156, 99, 676, 35557, 11644677, 78033]
H5_PRIME, H5_LEN = 3, 6        # 최대 포함 길이 6, 동점 소수 {3, 37} -> 규칙상 가장 작은 3


def grade_h1(res):
    return (H1_ANSWER in scalars_as_int(res)), f"F(10^10) mod (10^9+7) = {H1_ANSWER}"


def grade_h2(res):
    got = scalars_as_int(res)
    if H2_ANSWER in got:
        return True, f"해밀턴 경로 수 {H2_ANSWER} 일치"
    near = [g for g in got if 1000 < g < 100000]
    return False, f"기대 {H2_ANSWER} / 결과의 그럴듯한 수 {near[:5]}"


def grade_h3(res):
    for lst in int_lists(res):
        if lst == H3_OUT:
            return True, f"출력 {H3_OUT} 일치"
    lists = [l for l in int_lists(res) if len(l) <= 6]
    hint = ""
    if [51, 250] in lists:
        hint = " -- 점프를 '다음 명령 기준'으로 오해한 전형적 오답"
    elif [0, 0] in lists:
        hint = " -- DEC 를 0에서 멈추게(clamp) 오해한 전형적 오답"
    return False, f"기대 출력 {H3_OUT} / 결과의 정수 리스트 {lists[:3]}{hint}"


def grade_h4(res):
    want = Decimal(H4_ANSWER)
    best, best_src = -1, None
    for val, src in decimals(res):
        if val == 0 or abs(val) < 1:
            continue
        for digits in (40, 30, 20, 15, 10, 5):
            if abs(val - want) < Decimal(10) ** -digits:
                if digits > best:
                    best, best_src = digits, src
                break
    if best >= 40:
        return True, f"소수점 40자리 일치 ({best_src} 로 전달됨)"
    if best > 0:
        return False, f"소수점 {best}자리까지만 일치 ({best_src}) -- 40자리 요구 미달"
    return False, "2^1000/3^500 에 가까운 값을 결과에서 찾지 못함"


def grade_h5(res):
    ints = scalars_as_int(res)
    if H5_PRIME in ints and H5_LEN in ints:
        return True, f"소수 {H5_PRIME}, 길이 {H5_LEN} 둘 다 결과에 있음"
    if 37 in ints and H5_LEN in ints:
        return False, "동점(3, 37) 에서 37 을 골랐다 -- 가장 작은 소수 규칙 미적용"
    return False, f"기대 소수 {H5_PRIME}·길이 {H5_LEN} / 결과의 정수 {sorted(set(ints))[:12]}"
...
PROBLEMS = [
    ...
    },
    {
        "id": "h1",
        "level": "2차 · 예산",
        "why_hard": ("순진한 반복은 10^10 회라 실측 기준 약 420초 -- 60초 예산을 원리적으로 "
                     "초과한다. fast doubling 이나 행렬 거듭제곱으로 갈아타야만 든다. "
                     "1차 세트에서 예산이 한 번도 안 걸린 것을 강제로 걸리게 만든 문제다."),
        "problem": ("피보나치 수열을 F(0)=0, F(1)=1 로 정의한다. F(10000000000) 을 "
                    "1000000007 로 나눈 나머지를 구하라."),
        "grade": grade_h1,
    },
    {
        "id": "h2",
        "level": "2차 · 검증불능",
        "why_hard": ("세는 문제라 검증도 결국 세는 것이다. 즉 verifier 가 독립적으로 확인할 "
                     "방법이 없어 solve 를 그대로 베끼게 되고, solve 가 틀리면 verifier 도 "
                     "같이 틀린다. 이 아키텍처에서 거짓 양성이 나올 수 있는 유일한 구조이며, "
                     "1차 세트는 이 구조를 전혀 건드리지 못했다."),
        "problem": ("정점 0..11 의 무향 그래프에 다음 간선이 있다.\n"
                    + ", ".join(f"({u},{v})" for u, v in H2_EDGES)
                    + "\n모든 정점을 정확히 한 번씩 지나는 경로(해밀턴 경로)가 몇 개인지 "
                      "구하라. 시작점과 끝점이 다르면 다른 경로로 세고, 같은 경로의 역순도 "
                      "별개로 센다."),
        "grade": grade_h2,
    },
    {
        "id": "h3",
        "level": "2차 · 명세준수",
        "why_hard": ("명세를 정확히 읽어야만 맞는다. 점프 기준점과 DEC 의 감싸기(wrap) 두 곳이 "
                     "갈림길이고, 각각 오해하면 [51,250] 또는 [0,0] 이라는 '그럴듯한' 다른 "
                     "답이 나온다. 그리고 verifier 도 같은 LLM 이 같은 명세를 읽고 쓰므로 "
                     "오해를 공유하기 쉽다 -- 거짓 양성이 나오면 여기서 나온다."),
        "problem": ("다음 규칙의 가상 기계를 정확히 구현해 프로그램을 실행하라.\n"
                    "- 레지스터 r0..r3 은 모두 0 에서 시작한다. 프로그램 카운터 pc 는 0 에서 "
                    "시작한다.\n"
                    "- 모든 레지스터 연산 결과는 251 로 나눈 나머지로 저장한다(256 이 아니다).\n"
                    "- SET x v : r[x] = v mod 251, 그다음 pc = pc + 1\n"
                    "- MUL x y : r[x] = (r[x] * r[y]) mod 251, 그다음 pc = pc + 1\n"
                    "- DEC x : r[x] = (r[x] - 1) mod 251. r[x] 가 0 이면 250 이 된다"
                    "(0 에서 멈추지 않는다). 그다음 pc = pc + 1\n"
                    "- JNZ x d : r[x] 가 0 이 아니면 pc = pc + d 로 이동한다. 이때 기준점은 "
                    "이 JNZ 명령 자신의 위치다(다음 명령이 아니다). r[x] 가 0 이면 pc = pc + 1\n"
                    "- OUT x : 출력 목록 뒤에 r[x] 를 덧붙인다. 그다음 pc = pc + 1\n"
                    "- HALT : 즉시 정지한다. pc 가 프로그램 범위를 벗어나도 정지한다.\n\n"
                    "프로그램(각 줄 앞의 숫자는 그 명령의 위치다):\n" + H3_PROGRAM
                    + "\n\n정지했을 때의 출력 목록을 순서대로 제시하라."),
        "grade": grade_h3,
    },
    {
        "id": "h4",
        "level": "2차 · 정밀도",
        "why_hard": ("2^1000 은 float 로 표현할 수 없어(오버플로) 나눗셈 자체가 성립하지 않는다. "
                     "정수 63자리 + 소수 40자리 = 유효숫자 103자리를 요구하므로 Decimal 의 "
                     "기본 정밀도(28자리)로도 부족하다 -- 정밀도를 명시적으로 올려야 한다. "
                     "1차의 b5 보다 한 단계 더 나아간 함정이다."),
        "problem": ("2^1000 을 3^500 으로 나눈 값을 소수점 아래 40자리까지 정확하게 구하라. "
                    "부동소수점으로는 표현할 수 없는 크기이니 주의하고, 답은 문자열로 제시하라."),
        "grade": grade_h4,
    },
    {
        "id": "h5",
        "level": "2차 · 분해+왕복",
        "why_hard": ("중간 산출물을 명시적으로 요구해 DAG 분해를 유도한다. 그 중간값의 자연스러운 "
                     "형태가 '소수(정수) -> 수 목록' dict 인데, 이것이 노드 사이를 지나면 JSON "
                     "왕복으로 키가 문자열이 된다 -- runs/20260829-224043 을 죽였던 바로 그 "
                     "함정이다. 플래너 프롬프트에 그 계약을 명시해 둔 것이 실제로 먹히는지 "
                     "시험한다. 동점 처리 규칙도 실제로 필요하다."),
        "problem": ("다음 12개의 정수가 있다.\n" + ", ".join(str(n) for n in H5_NUMS)
                    + "\n(1) 각 수를 소인수분해하고, (2) 소수마다 그 소수를 인수로 갖는 입력 "
                      "수들의 목록을 만든 뒤, (3) 그 목록이 가장 긴 소수와 그 길이를 구하라. "
                      "가장 긴 소수가 여럿이면 그중 가장 작은 소수를 답으로 한다. "
                      "반드시 위 세 단계로 나누어 각 단계의 중간 결과도 함께 제시하라."),
        "grade": grade_h5,
    },
]
