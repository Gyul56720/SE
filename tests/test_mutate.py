...
try:
    ...
    ok(줄1["잰변형"] >= 1 and 줄1["Killed"] + 줄1["FG"] >= 1,
       f"요약이 잰 것을 담는다 (잰변형 {줄1['잰변형']} · Killed {줄1['Killed']} · FG {줄1['FG']})")
    ok(줄1["점수"] == round(줄1["Killed"] / (줄1["Killed"] + 줄1["FG"]), 4),
    ...
    ok(줄1["잰변형"] == 줄1["Killed"] + 줄1["FG"] + 줄1["못잼"],
       f"**잰변형 = Killed + FG + 못잼** -- 분모에서 빠진 수를 숨기지 않는다 "
       f"({줄1['잰변형']} = {줄1['Killed']} + {줄1['FG']} + {줄1['못잼']})")
    ...
    if 요3["잰변형"]:
        ok(abs(요3["덮음율"] - (요3["Killed"] + 요3["FG"]) / 요3["잰변형"]) < 5e-5,
           f"**덮음율 = (Killed+FG)/잰변형** ({요3['덮음율']}) -- 얼마나 제대로 쟀나")
    if 요3["Killed"] + 요3["FG"]:
        ok(abs(요3["잡음율"] - 요3["Killed"] / (요3["Killed"] + 요3["FG"])) < 5e-5,
    ...
    덜잼 = {"잰변형": 100, "Killed": 5, "FG": 5, "못잼": 90}
    덮 = (덜잼["Killed"] + 덜잼["FG"]) / 덜잼["잰변형"]
    ok(덮 < 0.2, f"못잼이 90%면 덮음율이 {덮:.2f} 로 떨어진다")
    ok(덜잼["Killed"] / (덜잼["Killed"] + 덜잼["FG"]) == 0.5,
       "**그런데 잡음율은 0.5 로 멀쩡해 보인다** -- 이것만 보면 못 잰 것이 안 보인다")

    print("\n== 완주 점검: 좋은 변형을 찾았나가 아니라 **쓸 만한 측정이었나** ==")
    좋 = {"잰변형": 10, "Killed": 4, "FG": 3, "못잼": 3, "씨앗": 0, "판": "abc1234",
         "사냥끝": True, "잡힌것": [f"k{i}" for i in range(4)],
         "결정된것": [f"k{i}" for i in range(4)] + [f"f{i}" for i in range(3)],
         "덮음율": 0.7, "잡음율": 0.5714}
    ok(M.완주점검(좋)["쓸만한가"], "다 맞으면 쓸 만하다")
    접힘 = {**좋, "잡힌것": ["k0", "k1"]}                  # Killed 4 인데 신원 2
    ㅅ = M.완주점검(접힘)
    ok(not ㅅ["쓸만한가"] and any(c["이름"] == "신원충돌" and not c["됐나"] for c in ㅅ["칸"]),
       "**잡힌것 수 < Killed 면 신원이 접힌 것이다** -- 셈과 신원을 맞춰 보면 그 자리에서 잡힌다")
    섞임 = {**좋, "결정된것": 좋["결정된것"] + ["못잼하나"]}   # 못잼이 판정 난 것에 섞였다
    ㅁ = M.완주점검(섞임)
    ok(not ㅁ["쓸만한가"] and any(c["이름"] == "못잼섞임" and not c["됐나"] for c in ㅁ["칸"]),
       "**결정된것 수 > Killed+FG 면 못잼이 섞인 것이다** -- 그러면 FR 이 퇴행으로 읽힌다")
