...
# **이 자만 본다.** 예전에는 check() 전체가 비는지를 봤는데, check() 는 길이 말고도
# 세계 확장 · 긴 대사 · 주고받기를 같이 본다 -- 그것들이 걸리면 길이와 무관하게 실패했다.
# 이 단언이 종료 블록 뒤에 있어서 아무도 못 봤다(G015 가 드러냈다).
ok(not [c for c in F.check(_ok, {}, {}) if "이 대목은" in c],
   "목표를 안 주면 이 자는 안 선다  ← 자와 프롬프트가 같이 움직인다")
...
# **글귀를 못박되, 지금 있는 글귀로 못박는다.** 아래 둘은 flow.py 에 없는 문장을
# 못박고 있었다 -- 프롬프트가 다시 쓰이면서 표현이 바뀌었는데, 종료 블록 뒤라 실패가
# 안 보였다. 뜻은 그대로 지키고 글귀만 지금 것으로 맞춘다.
ok("매번 같은 어미로 끝내지 마라" in src, "말끝을 다르게 끝내라고 한다")
# '뻘소리' 쪽은 flow.py 에서 아예 사라졌고 저장소 어디에도 없다(git log -S 로 확인).
# 대신 같은 일을 시키는 문장이 대사 지시에 남아 있다.
ok("용건만 오가는 대사가 제일 재미없다" in src,
   "용건 말고 딴 것도 하라고 한다  ← 예전 '뻘소리를 시킨다' 가 여기로 옮겨졌다")
