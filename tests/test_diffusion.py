...
THICK = "\n".join([
    ...
    '"그거 아버지 거였죠? 저 사진 뒤에 이름 적힌 거 내가 봤는데, 왜 여태 그런 거 없다고 하셨습니까."',
    '"몰라. 아버지 물건은 다 태웠다고 들었고 나는 그 자리에 있지도 않았으니까 나한테 묻지 마시고."',
    ...
    # **대사 줄의 여덟 할은 길다.** 짧게 받아치는 것은 앞말에 기대는 한 마디일 때뿐이다.
    '"그래서요."',
    '"그래서라뇨. 내가 사흘을 하나하나 세어 가면서 말했는데 그래서요, 라니. 그쪽은 그날 밤에 대체 어디 있었습니까."',
    '"아니 그러니까 내 말은, 그날 밤에 정확히 뭘 봤냐고 묻는 겁니다. 등이 꺼진 것 말고, 그 뒤에 물가에서요."',
    '"등을 끈 건 사람입니다. 스위치가 안에 있고 거기 들어갈 수 있는 사람은 셋인데, 둘은 뭍에 있었어요."',
    '"말을 하시죠."',
...
ok("용건만 오가는 대사가 제일 재미없다" in p, "용건만 오가는 대사를 막는다")
ok("읽히는 것이 먼저다" in p,
   "다만 읽히는 것이 먼저라고 못박는다  ← 이상하게 쓰려다 무슨 말인지 모르게 되면 실패다")
...
ok("뜻과 사건은 그대로 둔다" in src,
   "고칠 때 밀도를 깎지 못하게 한다  ← 지우는 것이 제일 쉬운 길이다")
...
print()
print("[대사] **줄의 여덟 할은 길다** -- 기본은 길게 말하는 것이다")
print("      ← 글자 몫(bulk)만 재면 아주 긴 것 하나로 몫을 채우고 나머지를 다 짧게")
print("        써도 통과한다. 그래서 대사가 문장처럼 짧아졌다. 이건 줄 수로 잰다.")
...
ok("묻지도 않은 것이 섞인다" in src, "정보를 섞으라고 한다")
# 느낌표 항목은 대사 규율을 한 곳으로 모으면서 접었다 -- 문장부호는 "한 대목에
# 한둘" 로 묶여 있다. 시키는 것이 많으면 안 지켜진다.
ok("문장부호가 잦으면 읽는 속도가 끊긴다" in
   __import__("novel.style", fromlist=["x"]).narrator(),
   "문장부호는 한둘로 묶는다")
