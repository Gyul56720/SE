...
짝최소 = 6                   # 짝이 이만큼은 있어야 부호검정이 뜻을 갖는다 (2^-6 = 1.6%)
...
def 짝으로갈렸나(바탕표본: "list[float]", 후보표본: "list[float]") -> bool:
    r"""**짝 전부에서 후보가 빨랐나** (부호검정). 짝이 `짝최소` 개 이상이어야 한다.

    우연히 모든 짝이 한쪽으로 쏠릴 확률은 2^-n 이다 -- 짝 일곱이면 0.78%. 가정을 세우지 않고
    셈으로 얻는 문턱이다.

    **실측 2026-09-13: 처음에는 짝을 안 보고 `max(후보) < min(바탕)`(분포 갈림)을 요구했다.**
    그랬더니 관문 13.94 -> 13.64초(2.1% · **일곱 짝 전부 후보가 빨랐다**)를 REJECT 했다 --
    4·5회차에 기계 부하가 올라가 양쪽이 같이 느려졌고, 그 때문에 *짝이 아닌* 최대·최소가
    겹쳤다. 번갈아 돌려 짝을 만들어 놓고 **짝 정보를 버린 것**이 잘못이었다."""
    이김, 짝 = 짝이김(바탕표본, 후보표본)
    return 짝 >= 짝최소 and 이김 == 짝
...
def 성능개선(바탕판, 후보판, 워크로드: str = "관문", 반복: int = 기본반복, V=None,
    ...
    if not 막힘 and 바닥 is None:
        ...
        이김, 짝 = 잰것["짝이김"]
        막힘.append(f"**짝이 한쪽으로 쏠리지 않았다** (후보가 이긴 짝 {이김}/{짝}, "
                   f"{짝최소}짝 이상에서 전부 이겨야 한다) -- 중앙값만 앞선 것은 아직 아니다")
    ...
    return {"결정": "ACCEPT", "V": True, "Δ": 잰것["Δ"], "J바탕": J바탕, "J후보": J후보,
            "까닭": [f"V(P') 섰다: {V말}",
                   ...
                   f"짝 {잰것['짝이김'][0]}/{잰것['짝이김'][1]} 전부 후보가 빨랐다)",
                   f"J = 1/t: {J바탕} -> {J후보}"],
            "잰것": 잰것}
