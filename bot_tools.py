...
def dispatch_command(command: str) -> str:
    """이 저장소의 **고정 명령(`!…`)을 네가 직접 친다** -- 사람이 자연어로 부탁하면 알맞은 명령을 골라 이걸로
    실행하고 결과를 붙여라. 명령 목록을 보여 주고 끝내지 마라. 예: 수집·틈 → `!수집 틈으로`, 논문 코드화 →
    ...
    **`!목표 승인`·`!계획 승인`·`!열쇠` 는 사람만 친다** -- 이 도구가 거절한다. 배경으로 도는 명령은 '시작' 만
    ...
    돼, 왜 = _d.도구로쳐도되나(command)
    if not 돼:
        relay.적기(f"⛔ 명령 거절 {command[:40]} -- {왜[:40]}")
        return f"[거절] {왜}"
