...
def 사용자개선(말: str, repo=None, 초: int = 180, 전부: bool = True, 전부초: int = 1800) -> dict:
    ...
    # 청하고, 그래도 빨가면 긴 호흡(조사)으로 넘긴다 -- 사람에게가 아니다.
    for 다시 in range(1, 시뮬되풀이횟수 + 1):
        if 시.get("통과"):
            break
        P.버림(repo)
        _적기(repo, {"꼴": "시뮬되풀이", "말": 말[:120], "다시": 다시, "꼬리": 시험보고[-300:]})
        되묻기 = (원프롬프트 + f"\n\n[레포 전체 시뮬 빨강 {다시}/{시뮬되풀이횟수}] 네 패치를 붙이고 저장소 검사를 전부 돌렸더니 "
                f"아래가 빨갛다. 고친 **패치 전체**를 같은 JSON 꼴로 다시 답하라(편집+새파일 다 포함).\n"
                + 시험보고[-1500:])
        try:
    ...
    if not 시.get("통과"):
        P.버림(repo)
        _적기(repo, {"꼴": "조사로", "말": 말[:200], "이유": "시뮬 빨강 " + str(r["시뮬시도"]) + "번"})
        r.update(판정="조사로", 말=f"레포 전체 시뮬이 {r['시뮬시도']}번 다 빨갛다 -- 긴 호흡(조사)으로 넘긴다\n" + 시험보고[-500:])
        return r
