...
class Novel:
    ...
    seed_id: str = ""
    # 세계 설정집(씨앗의 world). 게임의 설정집과 같다 -- 법칙·등급·종족·세력·의식주·
    # 시민의식·위협·잔혹. **디렉터가 매번 지어내지 않게** 하는 것이 목적이다. 설정이 비면
    # 씬마다 다른 세계가 나오고, 그것은 관문이 못 잡는 종류의 붕괴다.
    world: dict = field(default_factory=dict)
    # 이 소설이 시험하는 것(씨앗의 depth). **명제와 사상가 이름은 프롬프트에 싣지
    # 않는다** -- '칸트' 나 '정언명령' 이 들어가면 모델이 그 말을 텍스트에 쓰고, 그 순간
    # 소설이 강의가 된다. 실리는 것은 test(주인공이 하게 될 선택)와 cover(겉모습)뿐이다.
