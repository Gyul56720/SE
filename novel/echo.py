...
# **우리가 쓰는 말이 원고에 나오면 안 된다.** 프롬프트는 주인공을 "화자" 라 부르고
# 회차의 뼈대를 "비트" 라 부른다 -- 그것은 **우리끼리 쓰는 말**이지 이 세계의 낱말이
# 아니다. 실측 2026-09-09, 사용자 원고 1,115자: "화자" 가 **일곱 번** 나왔다.
#
#     밑바닥 구역의 진흙탕 속에서 눈을 뜬 화자가 …
#     결코 멈출 줄을 모르는 화자의 걸음.
#
# 이름이 없으면 모델은 **우리가 부르는 말을 이름으로 쓴다.** 디렉터가 칸 설명을 값
# 대신 베껴 내던 것(beat.echoed)과 같은 병이고, 그때는 카드에서 잡았는데 원고 쪽에는
# 그 자가 없었다.
#
# 흔한 낱말은 안 넣는다 -- "주인공" 은 인물이 소설 얘기를 하다 쓸 수 있고 "쾌감" 은
# 그냥 낱말이다. 여기 있는 것은 **원고에 나올 이유가 없는 말**만이다.
STAGE = ("화자", "비트", "갈고리", "전환점", "덩어리", "프롬프트", "원장", "회차")


def stage(text: str) -> list:
    """원고에 새어 나온 **무대 뒤의 낱말**. (낱말, 몇 번) 목록. 없으면 빈 것."""
    out = [(w, text.count(w)) for w in STAGE if w in text]
...
def check(text: str, prev: str) -> list[str]:
    ...
    if leak:
        w, n = leak[0]
        out.append(f"원고에 **무대 뒤의 낱말**이 나왔다 -- “{w}” {n}번"
                   + (f" (그리고 {len(leak) - 1}가지 더)" if len(leak) > 1 else "")
                   + ". 그건 우리가 쓰는 말이지 이 세계의 낱말이 아니다."
                     " **주인공을 이름이나 호칭으로 불러라** -- 이름이 아직 없으면 지어라.")
