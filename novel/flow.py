...
def _adopt(book: dict, llm, text: str) -> dict:
    ...
    # **그대로 복사한 줄은 지운다.** echo.check 가 이것을 이미 잡고 있었는데, 잡고
    # 나서 하는 일이 "장부에 적어 두고 원고는 그대로 쓴다" 였다. 그래서 재는 자는
    # 멀쩡한데 원고에는 같은 대목이 두 번 남는다 -- 실측 2026-09-09, 사용자가 보낸
    # 1화에 다섯 줄 279자(17%)가 통째로 두 번 적혀 있었다("이것도 이전에 쓴 소설이야").
    #
    # 기각이 아니다. 여기서 지우는 것은 **이 글 안에 이미 한 번 있는 줄**이라 읽는
    # 사람이 잃는 것이 없다 -- "과잉 기각은 글을 없앤다" 는 정도의 문제에 대한 말이고,
    # 통째로 복사한 줄은 정도의 문제가 아니다. 문턱 아래면 손대지 않는다(echo.dedup).
    #
    # **재기 전에 지운다.** 뒤에서 지우면 프로필과 장부가 복사된 글을 잰 값으로 남고,
    # 다음 덩어리는 있지도 않은 문제를 고치라는 말을 듣는다.
    text, _dup = echo.dedup(text)
    if _dup:
        D._log(f"[flow] 그대로 복사한 줄 {_dup:,}자를 지웠다 -- 첫 번째만 남긴다")
