...
- **같은 장면을 또 쓰지 마라.** 앞에서 이미 한 일(같은 자리에서 같은 사람과 같은 얘기)을
  다시 쓰려거든 **점층이 되게 하거나 아예 다른 데로 가라.** 굳이 되풀이해야 하면
  **조건을 바꿔라** -- 사람이 하나 빠졌거나, 시간이 지났거나, 한쪽이 이미 답을 안다.
  같은 것을 같게 쓰는 것은 분량이지 이야기가 아니다.
