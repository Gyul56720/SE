...
def ensure(book: dict, llm) -> "dict | None":
    ...
    n = _chars(book)
    card = book.get("card")
    # **회차 길이를 앞 회차가 끝난 자리에 안 매단다.** 전에는 전체 분량을 EP 로 나눠
    # 회차 번호를 셌다(`n // EP`). 그러면 앞 회차가 비트를 일찍 다 써서 짧게 끝났을
    # 때 그 남은 분량이 통째로 다음 회차에 얹힌다 -- 실측 2026-09-09: 1화 7,244자
    # 뒤에 2화가 11,502자로 불었다(EP 9,600). 회차가 하나 짧아지면 다음 하나가 그만큼
    # 길어지고, 그 긴 회차는 같은 비트 셋을 덩어리 넷에 늘여 쓴다 -- "전개가 없다".
    # 이제 **이 카드가 시작한 자리부터** 센다. 회차마다 EP 자다.
    if card and card.get("at") is not None:
        by_len = int(card.get("ep", 0)) + (n - int(card["at"])) // max(1, EP)
    else:
        by_len = n // max(1, EP)
...
def beat_span(book: dict) -> tuple:
    ...
    **회차 경계와 덩어리 경계는 안 맞는다.** 회차는 글자 수로 끊기고 덩어리는 모델이
    ...
    # **남은 분량은 이 카드로 센다.** 전에는 전체 분량을 EP 로 나눈 나머지로 셌는데,
    # 그 격자는 회차가 늘 EP 자마다 갈린다고 가정한다. 실제로는 마지막 비트를 일찍
    # 다 쓰면 회차가 짧게 끝나고(ensure 의 `done` 갈래), 그때부터 격자와 회차가
    # 어긋난다 -- 실측 2026-09-09 사용자 원고: 1화 7,244자 · 2화 11,502자(EP 9,600).
    # 1화가 남긴 2,356자가 통째로 2화에 얹혔다.
    left = max(0, EP - done)                            # 이 회차에 남은 분량
...
def last_chunk(book: dict) -> bool:
    ...
    beat_span 과 **같은 자로 잰다** -- 둘이 다른 자를 쓰면 마지막 비트를 시키는
    덩어리와 갈고리를 터뜨리는 덩어리가 어긋난다."""
    card = book.get("card") or {}
    total = _chars(book)
    done = max(0, total - int(card.get("at", total)))
    return max(0, EP - done) <= _chunk()
...
def brief(book: dict) -> str:
    ...
    # **같은 비트를 두 덩어리에 걸쳐 쓸 때가 있다.** 덩어리가 3,200자보다 짧게 나오면
    # 회차가 덩어리 넷을 먹고, 비트 셋을 넷에 나누면 하나가 두 번 걸린다(실측 2026-09-09
    # 시늉: 2,400자 덩어리로는 회차마다 하나씩 겹친다). 그런데 그때 앞 덩어리와 **글자
    # 하나 다르지 않은 지시**가 간다 -- 화자로서는 그 장면을 처음부터 다시 여는 것이
    # 맞는 읽기다. 사용자가 말한 "한 씬의 반복" 이 여기서도 난다.
    #
    # 위의 `k > 1` 줄은 **앞 비트**를 되풀이하지 말라는 말이라 이 자리를 못 덮는다 --
    # 여기서 되풀이되는 것은 앞 비트가 아니라 **지금 이 비트**다.
    _seen = c.setdefault("_비트시작", {})
    _nn = len(book.get("chunks") or [])
    if _seen.setdefault(str(k), _nn) < _nn:
        rows.append(f"  · **비트 {k}는 앞 덩어리에서 이미 시작했다.** 그 장면을 이어서 쓴다"
                    " -- 자리도 때도 그대로다. 다시 열지 말고, 앞 대목이 멈춘 바로 그"
                    " 순간부터 밀고 나가 이 비트를 끝낸다.")
    ...
    # **비트는 메모지 문장이 아니다.** 도착지(serial.brief)에는 "이 문장을 원고에
    # 옮겨 적지 마라" 가 있는데 비트에는 없었다. 그래서 카드의 "…한다" 가 원고에
    # 그대로 실렸다 -- 실측 2026-09-09 사용자 원고: 현재형 요약문 셋이 과거형 산문
    # 사이에 끼어 있었다("…중앙광장으로 향한다", "…검집을 확인한다", "…무릎을 꿇린다").
    # 시제가 어긋난 것이 곧 베낀 자국이다.
    rows.append("  · **위 비트는 무슨 일이 벌어지는지 적어 둔 메모다.** 그 문장을 원고에"
                " 옮겨 적지 마라 -- 메모는 ‘…한다’ 로 적혀 있고, 원고는 그 일이 **벌어지는"
                " 장면**이다. 사람이 움직이고 말하고 부딪치는 것으로 풀어라.")
