...
### 6단계. `/intent` + `/evolve` — 가장 위험하므로 맨 뒤 **(지어짐: intent/ · evolve/ · G020)**

지어진 것(2026-09-11): `intent/store.py` 목표 원장 -- 제안은 누구나(에이전트 포함),
**집기()는 승인 사건이 원장에 있는 목표만 돌려준다.** 승인 주체가 사람인 것은 관리
채널 화이트리스트가 지키고(공개 채널·에이전트 말로는 승인 불가), "끝났다" 는 말이
아니라 판정명령의 exit 0 이 정한다(없는 목표만 사람 선언 -- 관할 밖을 적는 규율).
승인된 목표는 digest(읽힐 텍스트)에 얹히고 승인 없는 제안은 안 올라온다.
`/evolve` 는 새 경로를 안 만들었다 -- self_challenge RED/GREEN → gates 승격이
그대로 진화 경로이고, 상한으로 **G020**(판정 원장 append-only · 게이트 삭제 금지,
파생 색인은 제외)을 그 경로 자신으로 증명해 승격했다(RED 2건 → GREEN → PROVEN=1;
승격 검사가 quotepath 버그까지 실측으로 잡아 재증명). `!목표` · `!진화`.
