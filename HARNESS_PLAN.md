...
### 7단계. 제2의 뇌 -- 참고를 모아 검증·압축하고, 값을 하는지 잰다

모델의 가중치는 못 만진다(API). 올릴 수 있는 것은 **문제 순간에 모델이 손에 쥐는 것**
이다. 그래서 수집기보다 **자(尺)** 를 먼저 짓는다 -- 모은 것이 값을 하는지는 같은 과제를
참고 있음/없음으로 풀어 맞힘 차이로만 안다.

1. **`eval/tasks` -- 절대 기준 과제 원장 (지어짐)**: 코드·추론·지식 12개, 판정은 코드
   (격리 실행 끝값 · 정규화 대조 · 낱말 포함). `--참고 둘다` 로 갈래별 이득, 준 참고의
   출처·해시가 원장에 남아 가지치기 재료가 된다. router 에 `풀이기` 역할.
2. **`dig/harvest` -- 수집기 (지어짐)**: 자가 틀린 과제의 깃발이 검색어. GitHub 저장소·
   코드 / HF 데이터셋·모델 → raw → 라이선스 허용 목록·compile 로 코드가 거름 → corpus
   (gitignore) + graph 색인(발췌, 지은이=코드) + 원장. 막힘은 멈추고 말한다. 하루 상한.
   `!수집` · systemd timer 2시간마다. 눈금: 추론·코드에 과제 8개 더해 천장을 뺐다.
3. 검증·압축 -- sandbox 실행 + graph/verify 대조 -> graph/store 에 깃발로.
4. 꺼내 쓰기 -- search_memory·delegate 가 문제 순간에 참고를 붙인다.
5. 효과 판정과 가지치기 -- 이득 ≤ 0 인 참고는 지운다(원장엔 "지웠다" 로 남긴다).
6. VM systemd timer 로 24시간 루프 (지어짐: deploy/se-harvest.timer, 2시간마다).

**보안 자가점검 (지어짐: secaudit/)** -- 사용자 규정 "제2의 뇌가 완성되면 내 노트북의
취약점을 dig→정제→제2의 뇌로 점검". 5단계(수집·정제·대조·점검·보고)를 잇되 판정은 코드가
규칙으로 낸다. 읽기 전용(포트·권한·SUID·계정·방화벽·키 노출), 남의 기계 공격·익스플로잇
없음. 못 본 것은 '못잼'(≠안전). `security_audit` 도구 · `!점검`.

**고치기 루프 (지어짐: repair/)** -- 사용자 규정 "문제 생기면 → tool 호출(dig·제2의 뇌) →
sandbox 시도 → … → 해결. 유저에게는 최소한의 요구만. 실패 이유는 기억 → night 압축".
재현 명령의 끝값이 판정하고 모델은 제안(패치/명령/사람/입력)만 한다. 바퀴 3(최대 5), 같은
제안 되풀이면 즉시 멈춤, 주어진 정보가 틀렸으면(이용자 측 과실) 그렇다고 고지하고 수정을
청한다. 실패 이유는 public_agent_memory 메모 → graph/night 가 장기기억으로, 못 푼 증상은
dig/harvest --틈 의 검색어로. `repair` 도구 · `!고치기`. 프롬프트: 사람에게 묻기 전에 (1) 이
루프가 먼저.

**곁가지 -- 수단이 없을 때 (지어짐: mailer.py · keys.py · `!열쇠` · `set_key` 도구)**: 실측 2026-09-11,
메일 부탁에 에이전트가 발급 절차를 설명하고 멈추고 → '인프라 없음' 으로 멈추고 → 선택지를
나열했다. 규칙을 코드로: 도구(send_email)가 "무엇이 없고 어떻게 주는지" 를 돌려주고,
사람은 `!열쇠 이름=값` 으로 딱 그것만 주며(값은 되비치지 않고 메시지는 지운다), 받으면
같은 호출이 바로 나간다. 사람만 할 수 있는 것(가입·2단계 인증·발급·결제)은 제일 짧은
길 하나로 청하고, 그 밖은 하네스가 한다.
