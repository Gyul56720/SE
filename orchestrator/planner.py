...
# 덜 낸다 -- 예산 초과는 attempts 에 남아 피드백으로 돌아오므로 '느림'도 수리 대상이 된다.
TIME_CONTRACT = (
...
def _computation_defect(run_dir: Path, code: str) -> str:
    ...
    왜 필요한가(실측 2026-09-03). 오케스트레이터가 텐서 분해를 다섯 판 내리 상수로 적는 쪽으로
    냈다. 인자행렬을 리터럴로 적고, 그것도 안 되면 영행렬을 할당해 놓고 채우지 못한 채
    끝냈다. 알고리즘 교체 0 회. 계산 시간 600 초와 이어쓰기 자리를 알려준 뒤에도 같았다.
    ...
    라이브러리 금지와 같은 층). 답은 계산해서 내야 한다. 상수로 적는 것도, 빈 배열도 아니다.
    mm333=22 처럼 아무도 모르는 값에서는 어차피 기억이 통하지 않으므로, 이 규칙이 무는
    자리는 외워서 넘길 수 있는 rung 뿐이다.

    패턴으로 완전히 막을 수는 없다(뜻 없는 반복문을 끼워 넣으면 통과한다). 그래도 "표를
    적으면 그대로 채택된다"와 "적으면 반려된다"는 다르고, 버전 추적이 실제로 무엇을
    했는지 계속 찍는다."""
    ...
    blocked = {"계산 없음(상수로 적음)", "계산 없음(빈 배열만)", "미완성(내용 없음)"}
...
def make_plan(problem: str, run_dir: str, pool=None, pool_id="planner", feedback: str = "",
    ...
    프롬프트에 되먹여 attempts 회까지 다시 시도한다. 이것이 없으면 계획 단계는 피드백이 없는
...
def repair_node(run_dir: str, node_id: str, pool=None, pool_id="planner") -> dict:
    ...
    # 것인지, 아예 다른 알고리즘으로 교체한 것인지"를 나중에 잴 수 없다. 그 구별이
    # 이 장치로 무엇을 관측하는가의 핵심이다 -- 피드백이 국소 수선만 하는 기계라면
