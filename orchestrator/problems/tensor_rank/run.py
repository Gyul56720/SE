...
[budget 을 오해하지 마라 -- 이것이 지금까지 가장 큰 실패 원인이다]
budget 은 **상한이지 목표가 아니다.** M <= budget 이기만 하면 만점이고, M 을 더 줄여도
점수가 오르지 않는다. **M = budget 을 정확히 채우는 것이 첫 목표다.**

실측으로 걸린 실패: budget=26 인데 M=23 으로 제출해서 73칸이 틀렸다. 26개를 쓸 수 있는데
스스로 23개로 줄여 문제를 훨씬 어렵게 만든 것이다. 3개를 더 쓰면 훨씬 쉬워진다.

**틀린 분해는 M 이 아무리 작아도 0점이다.** 정확성이 먼저고 개수는 그 다음이다.
확신이 없으면 항을 더 써라.
...
[case 가 무엇인가]
  ...
  · mm333    3x3 행렬곱 텐서. **최소 항 개수는 알려져 있지 않다.** 19 이상 23 이하까지만
             안다. 그 숫자는 배경 설명이지 네가 맞춰야 할 목표가 아니다 --
             **맞춰야 할 것은 이 case 의 budget 이다.**

행렬곱 텐서의 뜻: 색인을 a=(i,j), b=(j,k), c=(i,k) 로 펴면 M 항 분해가 곧 곱셈을 M 번만
쓰는 행렬곱 알고리즘이다. 알아야 풀리는 것은 아니지만, 항들이 어떤 구조를 갖는지에 대한
단서는 된다.
...
[반드시 알고 시작할 것 -- 이 문제가 사람을 속이는 방식]

① 경계랭크. 잔차를 0 으로 보내면서 계수를 발산시키면 **랭크보다 적은 항 수로 임의로
   가까워질 수 있다.** w_state 가 정확히 그 예다. 잔차 1e-12 는 답이 아니라 함정이다.
   그래서 허용치가 0 이고 성분에 격자가 걸려 있다. "거의 맞는 분해"는 분해가 아니다.

② 국소최소. ALS / 경사하강 계열은 이 목적함수에서 국소최소에 쉽게 갇힌다. 잔차가
   줄다 멈추고 계수가 커지기 시작하면 그것이 신호다. 무작위 재시작을 많이 하라 --
   한 번의 긴 수렴보다 짧은 시도 여러 번이 낫다.

③ SVD 로 차원을 줄이는 길은 **막혀 있다.** 세 전개행렬(unfolding)의 랭크를 재면
   mm333 은 (9, 9, 9) 로 전부 꽉 차 있다. HOSVD 로 줄일 코어가 없고, 그 값이 곧
   심판이 쓰는 하한이다. 차원축소로 얻을 것은 없으니 거기에 라운드를 쓰지 마라.

④ 격자가 좁다는 것은 제약이 아니라 **단서**다. 알려진 구성들의 성분은 작은 정수다.
   실수 전체를 훑는 대신 작은 정수 격자 위에서 탐색하는 편이 훨씬 낫다.

⑤ **정확한 것부터 손에 쥐고 시작하라.** 어떤 텐서든 0 이 아닌 칸 하나마다 항 하나씩
   쓰면 정확한 분해가 된다 -- 그 M 이 예산 이하면 그것으로 이미 통과다. 예산이 그보다
   작을 때만 줄이는 문제가 되고, 그때도 **이미 정확한 것을 손에 쥔 채로** 줄여야 한다.
   빈손에서 목표 개수를 한 번에 맞히려 들면 지금까지처럼 수십 칸이 틀린 채로 끝난다.
   **다만 줄이는 것은 항을 지우는 일이 아니다.** 자명한 분해의 항들은 서로 겹치지
   않는 표준기저라, 하나를 지우면 그 칸이 그냥 0 이 되고 남은 항으로는 메울 수 없다.
   항을 줄이려면 남은 항 전부를 다시 맞춰야 하고, 그것은 처음부터 그 개수로 푸는 것과
   같은 문제다. 개수를 줄인 분해는 항들이 **서로 상쇄되도록** 빽빽하게 생겼다 --
   자명한 분해 근처에 있지 않다. 그러니 개수를 하나 줄일 때마다 무작위 초기값으로
   여러 번 다시 맞춰라. 자명한 분해는 "적어도 이만큼은 확실하다"는 바닥일 뿐이다.
