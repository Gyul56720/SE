"""수리 버전들이 **같은 알고리즘을 미세조정한 것인지, 다른 알고리즘으로 교체한 것인지** 잰다.

왜 이것을 재는가. 피드백 루프가 국소 수선만 하는 기계라면 -- 상수를 바꾸고 반복 횟수를
늘리는 정도라면 -- 애초에 알려진 방법 밖으로 나갈 수 없다. 반대로 버전 사이에서 호출하는
...
두 버전의 거리는 **호출 이름 집합의 자카드 거리**로 잰다. 상수만 바뀐 버전은 거리 0 이고
호출 집합이 갈리면 1 에 가까워진다. 이것이 "미세조정"와 "알고리즘 교체"를 가르는 기준이다.

표지(marker)는 서술을 위한 것이지 판정 기준이 아니다. 어느 방식에도 안 걸리는 버전이
나오는 것이 오히려 이 실험이 바라는 결과다 -- 알려진 네 방식 밖이라는 뜻이므로.
"""
...
# 실측(2026-09-03, tensorrank mm333=22): 버전이 np.zeros 하나만 부르는데 추적기가
# "미분류 -- 알려진 방식 밖" 이라고 찍었다. 실제로는 23x9 영행렬을 할당해 놓고 채우지
# 못한 것이었다. 상수로 적은 코드를 걸러냈더니 이번에는 **빈 배열**이 같은 자리로 새어 들어왔다.
ALLOC = {
...
# 서술용 표지. 여기 없는 것이 나오면 "알려진 방식 밖"로 남긴다 -- 그것이 흥미로운 쪽이다.
MARKERS = {
...
def fingerprint(src: str) -> dict:
    ...
    # 도우미 함수 이름이 바뀐 것만으로 "알고리즘 교체"가 뜬다 -- 실측(2026-09-03)에서
    # get_mm222/get_mm333/get_w_state 라는 지역 함수 세 개가 등장한 것을 방법이
    # 통째로 바뀐 것으로 읽었다. 실제로는 상수로 적은 코드를 적어넣은 것이었다.
    ...
    if not tags:
        # **"알려진 방식 밖"가 두 가지를 뭉치고 있었다.** 알려진 방식 밖의 새 방법과, 아예
        # 방법이 없는 것(상수로 적은 코드를 그대로 적어넣기)은 완전히 다른 사건이다. 후자는
        ...
        tags = ["계산 없음(상수로 적음)"] if (not sig and dense) else \
               ["미완성(내용 없음)"] if not sig else \
               ["계산 없음(빈 배열만)"] if alloc_only else ["알려진 방식 밖"]
...
def versions(run_dir) -> list:
    """수리 버전을 시간순으로. history/<노드>/rNN.py 가 옛 버전, components/ 가 현재."""
...
def classify(prev: dict, fp: dict, d: float,
    ...
    그래서 **방식가 바뀌었는지**를 같이 본다. 거리가 크면서 방식도 갈렸을 때만
    알고리즘 교체로 부른다. 방식가 같은 채로 호출만 늘면 부분 교체다."""
    if d is None:
        return "첫 판"
    # 양쪽 다 실질 호출이 없으면 거리가 0 으로 나온다 -- 잴 것이 없기 때문이지 같아서가
    # 아니다. 골격에서 상수로 적는 쪽으로 간 것을 "미세조정"이라 부르면 거짓말이다.
    if not (prev.get("sig") or fp.get("sig")):
        return "성격 변화" if set(prev["tags"]) != set(fp["tags"]) else "미세조정"
    if d >= jump_at or (d >= near and set(prev["tags"]) != set(fp["tags"])):
        return "**알고리즘 교체**"
    return "미세조정" if d < tweak_at else "부분 교체"
...
def report(run_dir, **kw) -> dict:
    ...
    for name, path in versions(run_dir):
        ...
        prev = fp
    jumps = [r for r in rows if r["kind"] == "**알고리즘 교체**"]
    return {"versions": rows, "n_jumps": len(jumps),
            "families": sorted({t for r in rows for t in r["tags"]})}


def main() -> int:
    ap = argparse.ArgumentParser(description="수리 버전 사이의 알고리즘 점프를 잰다")
    ap.add_argument("run_dir")
    ap.add_argument("--calls", action="store_true", help="버전마다 호출 이름을 전부 찍는다")
    a = ap.parse_args()
    res = report(a.run_dir)
    if not res["versions"]:
        print("버전이 없다 -- 수리가 한 번도 돌지 않았거나 history/ 가 비었다")
        return 1
    print(f"{'버전':22} {'거리':>6}  {'구분':12} {'줄':>4} {'상수':>5}  방식")
    ...
    print(f"\n알고리즘 교체 {res['n_jumps']}회 · 등장한 방식: {', '.join(res['families'])}")
    if "알려진 방식 밖" in res["families"]:
        print("'알려진 방식 밖' 는 실질 호출이 있는데 알려진 방식에 안 걸린 것이다 -- 그쪽이 흥미롭다")
    if "계산 없음(빈 배열만)" in res["families"]:
        print("'계산 없음(빈 배열만)' 도 방법이 없는 것이다. 배열을 만들어 놓고 채우지 "
              "못한 것이므로 미분류로 세면 안 된다")
    if "계산 없음(상수로 적음)" in res["families"]:
        print("'계산 없음(상수로 적음)' 는 방법이 없는 것이다. 답을 적어넣은 것이지 찾은 것이 "
