...
def 바닥원인(gm: float = 5e-3, C_L: float = 25e-15, V_DD: float = 0.9,
         UI초: float = 50e-12, ADC비트: int = 7, 풀스케일시그마: float = 2.5,
         AFE입력rmsV: float = 0.067) -> dict:
    """**바닥을 정하는 것이 비교기인가 양자화인가** -- 둘을 같은 단위로 놓고 본다.

    비교기 감도는 `V_DD·e^{-UI·gm/C_L}`, ADC 계단은 `2·rho·rms/2^B` 다. 둘을 볼트로
    옮겨 견주면 어느 쪽이 지배하는지 바로 보인다. **더 큰 쪽이 바닥을 정한다.**
    """
    비교기 = strongarm(gm=gm, C_L=C_L, V_DD=V_DD, UI초=UI초)["감도V"]
