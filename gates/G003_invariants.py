...
REQUIRED: "list[tuple[str, str, str]]" = [
    ...
    # 2026-09-11: 호출이 commit_guard 로 옮겨졌다(게이트·바뀐 파일의 검사·main CI 를 한 문에서).
    # 고정점은 두 줄이 됐다 -- git_sync 가 문지기를 부르고, 문지기가 게이트를 부른다.
    ("discord_bot_server.py", "commit_guard.검사",
     "커밋 전 강제 문지기 호출 -- 이 한 줄이 사라지면 게이트·검사·CI 문이 전부 무력화된다"),
    ("commit_guard.py", "gatekeeper.run_gates",
     "문지기 안의 게이트 호출 -- 이 한 줄이 사라지면 모든 게이트가 무력화된다"),
