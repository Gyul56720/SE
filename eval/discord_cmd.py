...
def _돌고있나() -> str:
    p = subprocess.run(["pgrep", "-af", "eval/run.py"], capture_output=True, text=True)
...
def run(text: str, runner=None, allow_write: bool = True) -> "str | None":
    ...
    if words and words[0] == "전부":
        ...
        로그.parent.mkdir(parents=True, exist_ok=True)
        with open(로그, "ab") as f:
            subprocess.Popen(["python3", "eval/run.py", "--전부"], cwd=str(REPO),
                             stdout=f, stderr=subprocess.STDOUT,
                             stdin=subprocess.DEVNULL, start_new_session=True)
        살아 = _돌고있나()
        if not 살아:
            return "띄웠는데 pgrep 에 안 보인다 -- 시작했다고 말하지 않는다. 로그를 보라: " \
                   f"{로그}"
        return (f"백그라운드로 시작했다 (pgrep 확인됨): {살아[:100]}\n"
                f"로그: {로그} · 결과는 `{PREFIX} 상태` 로")
