...
def 모름_가르기(모름: list) -> list:
    ...
    8개가 어느 칸에도 안 들어갔다. 답이 양쪽에 다 있는데도 모름이 나오는 길이 있다 --
    옮김이 터지거나 부모 판정기가 안 돌면 확장이 None 이 되고, 그러면 도약도 재작성도
    딴 문제도 아니라서 모름으로 떨어진다.

    그 8개는 **덜 푼 것이 아니라 고장난 것**이다. sweep 을 아무리 돌려도 안 줄어든다.
    같은 칸에 섞어 두면 "더 풀면 되겠지" 로 읽힌다 -- 그래서 갈라 놓는다.
