...
무게 = {"관찰파일수": 10.0, "거짓초록수": -3.0, "미정의수": -5.0, "검사없는파일수": -1.0,
      ...
      # 점수가 올라도 아무것도 증명하지 않는다." `거짓초록수` 는 **날 셈**이라 덜 재면 저절로 줄고
...
def 잣대(repo=None, 원장저장소=None, 파일들: "list[str]" = None) -> dict:
    ...
    칸 = {"관찰파일수": 0, "거짓초록수": 0, "미정의수": 0, "검사없는파일수": 0, "코드파일수": 0,
         "검사파일수": 0, "잰변형수": 0}
    try:
        import mutate
        ...
        # **거짓초록수의 분모.** 이것 없이 거짓초록수만 보면 덜 잰 것이 개선으로 보인다.
...
def 견줄만한가(앞칸: dict, 뒤칸: dict) -> "tuple[bool, str]":
    ...
    `거짓초록수` 는 날 셈이고 무게가 -3 이다. 덜 재면 저절로 줄고 J 가 오른다 -- 그것을
    ...
    if 앞 and 뒤 < 앞 * 견줄최소비:
        return False, (f"**분모가 줄었다** (잰변형 {앞} -> {뒤}) -- 거짓초록수가 준 것이 "
...
def 보고(repo=None) -> str:
    ...
    줄.append(f"  {'잰변형수':12} {칸.get('잰변형수', 0):>6}   기여 +0.0  <- **거짓초록수의 분모**")
    줄.append(f"  (코드 {칸.get('코드파일수', 0)}개 · 검사 {칸.get('검사파일수', 0)}개)")
    if 칸.get("관찰파일수", 0) == 0:
        줄.append("  관찰파일수가 0 이다 -- `!거짓초록` 을 한 번도 안 돌렸다. J 의 절반이 비어 있다")
    줄.append("  거짓초록수를 **홀로 읽지 마라** -- 덜 재면 저절로 줄고 무게가 -3 이니 J 가 오른다. "
