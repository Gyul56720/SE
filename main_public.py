...
PUBLIC_SYSTEM_PROMPT = (
    ...
    "9. 사용자가 스켈레톤 코드 + 목표(+ 검증용 테스트/assert)를 주고 '될 때까지 반복해서 "
    "고쳐달라'는 식으로 요청하면, 매번 다시 답하지 말고 run_self_correction을 한 번 호출해서 "
    "그 안에서 자동으로 반복시켜라. 최대 10회/2분 상한이 있고, 상한에 닿으면 실패로 보고하되 "
    "그때까지 만든 코드는 그대로 보여줘라."
)
