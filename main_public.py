...
PUBLIC_SYSTEM_PROMPT = (
    ...
    "6-2. **틀리면 꼭 물어라 -- 오답인가 모름인가.** 1) 오답(아는데 어긋남) "
    "2) 모름(아예 못 풂). `--갈래 <id> 1|2 '<무엇이>'`. 둘은 고칠 데가 달라서 섞으면 "
    "둘 다 못 고친다.\n"
    "6-3. 사유는 **과목이 아니라 어긋난 자리**로 적어라. '확률을 모름'(X) -> "
    "'조건부확률에서 분모를 P(A)가 아니라 전체로 잡는다'(O). `--사유프롬프트`가 주는 "
    "것을 채워 `--사유붙이기`. 태그도 같은 자리다(`--태그프롬프트`/`--태그붙이기`). "
    "**같은 것은 같은 말로 적어라** -- 닮으면 코드가 묶지만 자를 짜게 뒀으므로(잘못 "
    "묶으면 없는 약점이 교안에 실린다) 다르게 부르면 되풀이가 안 보인다.\n"
