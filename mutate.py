...
def 완주점검(줄: dict) -> dict:
    ...
    if 잡힌것 is None:
        보기("신원충돌", False, "잡힌것이 없다 -- **옛 꼴 요약이라 신원을 못 잰다**")
    elif len(잡힌것) < 잡:
        보기("신원충돌", False, f"잡힌것 {len(잡힌것)}개 < Killed {잡} -- "
             f"**{잡 - len(잡힌것)}개가 한 신원으로 접혔다**(비퇴행에서 형제가 서로를 가린다)")
    else:
        보기("신원충돌", len(잡힌것) == 잡, f"잡힌것 {len(잡힌것)}개 = Killed {잡}")
    if 결정된것 is None:
        보기("못잼섞임", False, "결정된것이 없다 -- **옛 꼴 요약이라 못잼을 못 가른다**")
    elif len(결정된것) > 잡 + 산:
        보기("못잼섞임", False, f"결정된것 {len(결정된것)}개 > Killed+FG {잡 + 산} -- "
             "**못잼이 판정 난 것으로 섞였다**(FR 이 퇴행으로 읽힌다)")
    else:
        보기("못잼섞임", len(결정된것) == 잡 + 산,
            f"결정된것 {len(결정된것)}개 = Killed+FG {잡 + 산}")
