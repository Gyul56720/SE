...
def 비퇴행(앞요약: dict, 뒤요약: dict) -> dict:
    ...
    **뒤 판이 다시 잰 것만 따진다.** 시한에 잘려 안 닿은 변형을 '놓쳤다' 고 하면, 그것은
    재지 않은 것을 빨강이라 하는 것이다 -- 이 저장소가 거짓빨강이라 부르는 바로 그 잘못이다.
    ...
    if not 앞잡:
        return {"비퇴행": "못잼", "겹친수": 0, "놓친": [],
                "말": "앞 판에 잡힌 변형의 신원이 없다 -- 견줄 바탕이 없다"}
    if not 뒤잰:
        return {"비퇴행": "못잼", "겹친수": 0, "놓친": [],
                "말": "뒤 판에 잰 변형의 신원이 없다 -- 무엇을 다시 쟀는지 모른다"}
    겹 = 앞잡 & 뒤잰
    if not 겹:
        return {"비퇴행": "못잼", "겹친수": 0, "놓친": [],
                "말": f"앞이 잡던 {len(앞잡)}개 가운데 뒤가 다시 잰 것이 **하나도 없다** -- "
                    "표본이 안 겹치면 퇴행인지 아닌지 모른다(안 잰 것을 빨강이라 하지 않는다)"}
    놓친 = sorted(겹 - 뒤잡)
    if 놓친:
        return {"비퇴행": "퇴행", "겹친수": len(겹), "놓친": 놓친,
                "말": f"**다시 쟀는데 {len(놓친)}개를 놓쳤다** (겹친 {len(겹)}개 중) -- "
                    f"검증 능력이 줄었다. 예: {놓친[0]}"}
    return {"비퇴행": "지킴", "겹친수": len(겹), "놓친": [],
            "말": f"앞이 잡던 것 가운데 뒤가 다시 잰 {len(겹)}개를 **전부 다시 잡았다**"}
