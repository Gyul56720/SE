...
expected_sum = sum(ascii_vals)

print(f"=== Input Sentence: '{sentence}' ===")
print(f"=== ASCII Values: {ascii_vals} ===")
print(f"=== Expected Sum (Weights = +1): {expected_sum} ===")
...
module npu_sentence_testbench;
    ...
    initial begin
        ...
        wait(npu_done);
        $display("HARDWARE_RESULT=%0d (Expected Sum: {expected_sum})", npu_result);
        $finish;
    end
endmodule
"""

with open("npu_sentence_testbench.sv", "w") as f:
    f.write(sentence_tb_code.replace("{expected_sum}", str(expected_sum)))

subprocess.run(["iverilog", "-g2012", "-o", "sentence_sim", "npu_axi_top.sv", "npu_sentence_testbench.sv", "npu_array_4096.sv", "npu_tile.sv", "npu_pe.sv"], check=True)
res = subprocess.run(["vvp", "sentence_sim"], capture_output=True, text=True)
print(res.stdout)
