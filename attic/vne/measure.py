...
이것이 성립해야 분모에서 빠진 수를 숨길 수 없다. 이 저장소가 반례 사냥에서
...
def _새쪽(방법: str) -> dict:
    """한 방법이 한 흐름을 도는 동안 들고 다니는 것. `한판` 과 `짝판` 이 같이 쓴다."""
    return {"방법": 방법, "배치기": EM.방법들.get(방법),
            "쓴cpu": {}, "쓴대역": {},
            "살아있는": [],                               # (나갈때, 요청, 배치)
            "칸": [],                                     # 요청마다 하나 -- 짝 비교의 단위
            "셈": {"받음": 0, "거절": 0, "못잼": 0}, "사유": {}}
...
def _한요청(바탕, 쪽: dict, 순번: int, 들어온때, 나갈때, 요청, 시한초: float) -> None:
    """요청 하나를 그 쪽의 잔여 자원에 얹고 **시간을 잰다.** 칸 하나를 남긴다."""
    # **수명이 끝난 것을 먼저 돌려준다.** 안 그러면 뒤로 갈수록 전부 거절이라
    # 수용률이 방법이 아니라 요청 수의 함수가 된다.
    남은것 = []
    for 끝, 옛요청, 옛배치 in 쪽["살아있는"]:
        if 끝 <= 들어온때:
            EM.자원깎기(바탕, 옛요청, 옛배치, 쪽["쓴cpu"], 쪽["쓴대역"], 부호=-1)
        else:
    ...
    try:
        d = 쪽["배치기"](바탕, 요청, 쪽["쓴cpu"], 쪽["쓴대역"])
        걸림 = time.perf_counter() - t0
        if 걸림 > 시한초:
            쪽["셈"]["못잼"] += 1
            쪽["칸"].append({"순번": 순번, "결과": "못잼", "왜": "시한초과", "초": round(걸림, 6)})
            return
    except Exception as e:                                     # noqa: BLE001
        걸림 = time.perf_counter() - t0
        쪽["셈"]["못잼"] += 1
        쪽["칸"].append({"순번": 순번, "결과": "못잼", "왜": f"{type(e).__name__}: {e}"[:120],
                       "초": round(걸림, 6)})
        return

    if d["됐나"]:
        EM.자원깎기(바탕, 요청, d, 쪽["쓴cpu"], 쪽["쓴대역"], 부호=1)
        쪽["살아있는"].append((나갈때, 요청, d))
        쪽["셈"]["받음"] += 1
        쪽["칸"].append({"순번": 순번, "결과": "받음", "매출": d["매출"], "비용": d["비용"],
        ...
        쪽["셈"]["거절"] += 1
        쪽["사유"][d["사유"]] = 쪽["사유"].get(d["사유"], 0) + 1
        쪽["칸"].append({"순번": 순번, "결과": "거절", "왜": d["사유"], "초": round(걸림, 6)})


def _판(쪽: dict, 씨앗: int, 꼴: str, 바탕, 시한초: float, 온: int, 짝지어쟀나: bool = False) -> dict:
    """한 쪽의 칸들을 표본 벡터로 접는다."""
    칸 = 쪽["칸"]; 셈 = 쪽["셈"]
    ...
    return {
        ...
        "방법": 쪽["방법"], "씨앗": 씨앗, "꼴": 꼴, "바탕노드수": len(바탕.노드),
        "바탕링크수": len(바탕.링크), "시한초": 시한초, "기계": _기계(),
        # **어떻게 쟀나**가 표본에 남아야 한다 -- 따로 잰 시간과 번갈아 잰 시간은 같은 자가 아니다
        "짝지어쟀나": 짝지어쟀나,
        # ---- 표본: **수용률보다 먼저 읽을 것들** ----
...
def 한판(방법: str, 씨앗: int = 0, 요청수: int = 기본요청수, 바탕노드수: int = T.바탕노드수,
    ...
    **시간을 견주려면 이것 둘을 쓰지 말고 `짝판` 을 써라** -- 까닭은 거기 적혀 있다."""
    말 = 말하기 or (lambda s: None)
    if EM.방법들.get(방법) is None:
    ...
    for 순번, (들어온때, 나갈때, 요청) in enumerate(흐름):
        _한요청(바탕, 쪽, 순번, 들어온때, 나갈때, 요청, 시한초)
        if 순번 % 50 == 49:
            말(f"[vne] {순번 + 1}/{요청수} 받음 {쪽['셈']['받음']} 거절 {쪽['셈']['거절']} "
              f"못잼 {쪽['셈']['못잼']}")
    return _판(쪽, 씨앗, 꼴, 바탕, 시한초, len(흐름))


def 짝판(앞방법: str, 뒤방법: str, 씨앗: int = 0, 요청수: int = 기본요청수,
       바탕노드수: int = T.바탕노드수, 꼴: str = "waxman", 시한초: float = 요청시한초,
       말하기=None) -> "tuple[dict, dict]":
    r"""두 방법을 **한 흐름 안에서 요청마다 번갈아** 잰다. 시간을 견주려면 이것을 써야 한다.

    **실측 2026-09-14 -- 이 자리가 거짓말을 하고 있었다.** `한판` 을 두 번 따로 돌리고 나서
    `짝` 이 순번으로 짝지었다. 그러면 **짝이 시간상 붙어 있지 않다** -- 앞 판이 다 끝난 뒤에
    뒤 판이 돈다. 그 사이 기계가 흔들리면(주파수·GC·남의 부하) 흔들림이 **모든 짝을 같은
    쪽으로** 민다. 같은 방법끼리 견줘 재 봤다:

        탐욕 vs 탐욕 · 30짝 · 12번 되풀이
        뒤가 이긴 짝: 11 13 3 15 13 10 5 8 10 11 3 3   -- 평균 8.8 / 30

    공정하면 15 언저리여야 한다. **뒤가 늘 느렸다.** 그래서 `tests/test_vne.py` 의
    "같은 방법끼리는 못 가른다" 가 세 번에 한 번씩 빨개졌다 -- 검사가 흔들린 게 아니라
    **자가 한쪽으로 기울어 있었다.** 그 자로 잰 "FF 가 느리다" 도 같은 만큼 의심스럽다.

    고치는 법은 통계가 아니라 **배치**다. 요청 하나마다 두 방법을 잇따라 재면 드리프트가
    짝 안에서 상쇄된다. 그리고 **어느 쪽을 먼저 재는지도 순번마다 뒤집는다** -- 짝 안에서도
    먼저 재는 쪽이 유리하면 그것 역시 한쪽으로 쏠린다.

    자원은 쪽마다 따로 든다(`_새쪽`). 같은 바탕망·같은 흐름을 보되, 무엇을 물고 있는지는
    방법마다 다르다 -- 온라인 VNE 라 그게 본질이다."""
    말 = 말하기 or (lambda s: None)
    for m in (앞방법, 뒤방법):
        if EM.방법들.get(m) is None:
            return ({"방법": m, "못잼": -1, "말": f"모르는 방법 '{m}' ({', '.join(EM.방법들)})"}, {})
    바탕 = T.바탕망(씨앗=씨앗, 노드수=바탕노드수, 꼴=꼴)
    흐름 = T.요청흐름(씨앗=씨앗, 개수=요청수)
    앞쪽, 뒤쪽 = _새쪽(앞방법), _새쪽(뒤방법)
    for 순번, (들어온때, 나갈때, 요청) in enumerate(흐름):
        차례 = (앞쪽, 뒤쪽) if 순번 % 2 == 0 else (뒤쪽, 앞쪽)   # 짝 안의 순서도 뒤집는다
        for 쪽 in 차례:
            _한요청(바탕, 쪽, 순번, 들어온때, 나갈때, 요청, 시한초)
        if 순번 % 50 == 49:
            말(f"[vne] {순번 + 1}/{요청수} {앞방법} {앞쪽['셈']['받음']} · "
              f"{뒤방법} {뒤쪽['셈']['받음']}")
    온 = len(흐름)
    return (_판(앞쪽, 씨앗, 꼴, 바탕, 시한초, 온, 짝지어쟀나=True),
            _판(뒤쪽, 씨앗, 꼴, 바탕, 시한초, 온, 짝지어쟀나=True))
...
def 짝(앞: dict, 뒤: dict) -> dict:
    ...
    return {
        ...
        # **비긴 짝은 부호가 없어 표본에서 빠진다.** 몇이 빠졌는지 안 숨긴다
        "짝비김": perf.짝비김(앞초, 뒤초),
...
def 여러씨앗(앞방법: str, 뒤방법: str, 씨앗들, 요청수: int = 기본요청수,
    ...
    **중앙값은 n 에 거의 안 움직이고 씨앗에 다섯 배로 움직인다.** 그 +0.0380 은 하필
    ...
    각각, 나쁨 = [], []
    for s in 씨앗들:
        # **번갈아 잰다.** 따로 두 번 돌리면 그 사이의 기계 드리프트가 모든 짝을 같은 쪽으로
        # 민다 -- `짝판` 의 실측을 보라. 수용률은 어느 쪽으로 재도 같다(결정적이다).
        앞, 뒤 = 짝판(앞방법, 뒤방법, 씨앗=s, 요청수=요청수, 바탕노드수=바탕노드수, 꼴=꼴)
...
def 견줌보고(앞: dict, 뒤: dict) -> str:
    ...
    return "\n".join([
        ...
        f"짝 {r['짝이김'][0]}/{r['짝이김'][1]} · 비김 {r.get('짝비김', 0)} · 부호검정 p {p글}",
        _시간말(r, 바닥),
    ])
...
def main(argv=None) -> int:
    ...
    if a.cmp:
        판 = 짝판(a.cmp[0], a.cmp[1], 씨앗=a.씨앗, 요청수=a.요청, 바탕노드수=a.노드,
               꼴=a.꼴, 말하기=lambda s: print(s, flush=True))
        print()
        print(견줌보고(*판))
