...
/**
 * NPU AXI4/ACE-Lite Master Interface (Secure & Coherent)
 * Implementation based on Senior Engineer Guidelines.
 */
module npu_axi_top (
    input  logic                    clk,
    input  logic                    rst_n,
    ...
    // High-Bandwidth AXI Memory Write Interface (Slave Port for weights/acts)
    ...
    // Inline Crypto Engine (Tessera Architecture)
    ...
    // [1] Internal Memory with Outstanding Support
    logic signed [7:0] act_mem [4095:0];
    logic        [1:0] weight_mem [4095:0];

    // [2] Inline Decryption (AES-CTR concept: real-time XOR)
    logic [127:0] decrypted_wdata;
    assign decrypted_wdata = s_axi_mem_wdata ^ crypto_key;

    // [3] Registered Handshaking (No Handshake Dependency Violation)
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (s_axi_mem_awvalid && s_axi_mem_wvalid && r_mem_awready && r_mem_wready) begin
                r_mem_bvalid <= 1'b1;
                // Write Address Splitter & 4KB boundary checks would go here for a Master.
                // Since this is a Slave port receiving data:
                if (s_axi_mem_awaddr >= 32'h1000) begin
                    for (int i=0; i<64; i++) weight_mem[((s_axi_mem_awaddr-32'h1000)*4)+i] <= decrypted_wdata[i*2 +: 2];
                end else begin
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (s_axi_csr_arvalid && s_axi_csr_arready) begin
                s_axi_csr_rvalid <= 1'b1;
                case (s_axi_csr_araddr[7:0])
                    8'h00: s_axi_csr_rdata <= {31'b0, reg_done};
    ...
    // [5] NPU Core Pipelined Instantiation
