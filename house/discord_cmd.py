...
def run(text: str, runner=None, allow_write: bool = True) -> "str | None":
    ...
    if 머리 in ("설계", "구상", "design", "새회로"):
        ...
        # 제안 블록은 "외부 아날로그" 하나. 보고서 자신이 "13개가 비어 있다" 고 정직하게
        # 적었지만, 그 정직함이 비용을 없애 주지는 않는다.
        #
        # 여기서 막는 것이 싸다. 모델을 부르기 전이고, 사람은 한 줄만 더 쓰면 된다.
        sys.path.insert(0, str(REPO))
        from house import spec as _SPEC
        _s = _SPEC.읽기(요청)
        # **모델이 있으면 막지 않는다.** 규칙 판독이 비어도 `spec.다듬기` 가 모델로
        # 그 칸을 읽는다(2026-09-22). 낱말표에 없는 IP 마다 사람이 표를 고쳐 주어야
        # 한다면 그것은 봇이 혼자 도는 것이 아니다 -- 사용자가 요구한 것이 그것이다.
        # 모델도 없고 규칙도 못 읽었을 때만 막는다. 그때는 정말로 아무도 못 읽는다.
        from house import gen as _GEN
        _모델있나 = bool((_GEN.쓸수있나() or {}).get("모델키"))
        if not (_s.쓰임새 or _s.문제 or _s.회로 or _s.수) and not _모델있나:
            return ("**요청에서 아무것도 못 읽었습니다 — 돌리지 않았습니다.**\n"
