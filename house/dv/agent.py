...
def 일하기(규모="보통", 설계=None) -> dict:
    ...
    # --- 테스트벤치가 무엇을 내는지 먼저 본다 ---
    #
    # **없는 키를 0 으로 채우지 않는다.** 예전에는 `d["cov_pct"]` 처럼 바로 꺼내서,
    # FIR 이 아닌 회로를 넘기면 KeyError 로 죽었다(실측 2026-09-24, aim_chain).
    # 죽는 것보다 나쁜 길은 0 으로 채우는 것이다 -- 커버리지 0 % 가 그려지고 그것이
    # '검증이 약하다' 로 읽힌다. **안 낸 것은 안 냈다고 적고 그 절을 뺀다.**
    필수 = ("txn", "pass", "fail", "timeout", "clk_cycles")
    선택 = ("cov_pct", "sat_txn", "cyc_med", "cyc_max", "proto_err",
          "cov_sat", "cross_mag_sat", "directed", "cov_hit", "cov_all")
    첫 = SIM.돌리기(None, seed=99, txn=설정["닫기"][0], maxlen=256, cap=200000, 설계=d0)
    빠진필수 = [k for k in 필수 if k not in 첫]
    if 빠진필수:
        raise RuntimeError(
            f"{d0.키}: 테스트벤치가 필수 키를 안 낸다 -- {', '.join(빠진필수)}. "
            f"({d0.TB}) 검증은 이 키들 없이는 아무 말도 할 수 없다.")
    R["미보고"] = [k for k in 선택 if k not in 첫]
    ...
    for s in range(1, 설정["회귀씨앗"] + 1):
        ...
        회귀.append({"seed": s, "pass": d["pass"], "fail": d["fail"], "timeout": d["timeout"],
                   "proto": d.get("proto_err", 0), "cov": d.get("cov_pct"),
                   "주기": d["clk_cycles"], "초": d["_초"], "sat": d.get("sat_txn"),
                   "cyc_med": d.get("cyc_med"), "cyc_max": d.get("cyc_max")})
    R["회귀"] = 회귀
    ...
    # --- 5. 자해 검사 ---
    t1 = time.time()
    # **변이에 회로를 물린다.** 예전에는 설계를 안 넘겨서 어떤 회로를 맡겨도 FIR 의
    # RTL 에 변이를 걸었다 -- 다른 회로의 보고서에 FIR 의 자해 검사 결과가 실렸다.
    R["변이"] = MUT.한바퀴(txn=설정["변이거래"], 씨앗들=(101, 102, 103), 지시=20, 설계=d0)
...
def 보고서(m: dict) -> RPT.보고서:
    ...
    # **표지의 시간이 부록의 시간과 같아야 한다** (RTL 보고서와 같은 고침).
    R.업무초 = m.get("초")
    ...
    # **이 회로의 테스트벤치가 안 내는 것.** 없는 절은 빼되, 왜 뺐는지 적는다.
    미보고 = set(m.get("미보고") or [])
    커버있나 = "cov_pct" not in 미보고 and bool(m.get("닫기곡선"))
    포화있나 = "sat_txn" not in 미보고
    끝cov = m["닫기곡선"][-1]["cov"] if 커버있나 else None
    ...
    if 커버있나:
        ...
        R.요약("<b>이 회로의 테스트벤치는 기능 커버리지를 내지 않는다</b> — "
              f"안 내는 것: {', '.join(sorted(미보고)) or '(없음)'}. "
              "커버리지 절(§4)을 뺀다. <b>0 %로 채워 그리지 않는다</b> — "
              "안 잰 것을 그리면 '검증이 약하다' 로 잘못 읽힌다")
  ...
  ※ 포화를 마지막에 한 번만 하면 답이 달라진다. 중간에 넘쳤다가
    ...
    R.표(["항목", "값"],
        [["시드 수", f"{len(회)}"],
         ...
         ["포화가 일어난 거래",
          (f"{sum(x['sat'] for x in 회):,}" if 포화있나 else "(이 회로의 TB 는 안 낸다)")],
    ...
    # ---------------- 4. 커버리지 닫기 ----------------
    # **커버리지를 안 내는 회로가 있다.** 그 절을 0 %로 채워 그리지 않는다 --
    # 안 잰 것을 그리면 "검증이 약하다" 로 잘못 읽힌다. 없으면 없다고 적고 뺀다.
    if not 커버있나:
        R.절("4. 커버리지 닫기 — 이 회로는 커버리지를 내지 않는다")
        R.글("이 회로의 테스트벤치(<code>%s</code>)는 기능 커버리지를 보고하지 않는다. "
             "안 내는 것: <code>%s</code>. 그래서 닫기 곡선과 시드별 커버리지를 "
             "<b>그리지 않는다</b> — 0 %%로 채우면 안 잰 것이 재어진 것처럼 보인다. "
             "커버리지를 말하려면 이 회로의 커버포인트를 먼저 정의해야 한다."
             % (m.get("설계이름", "?"), ", ".join(sorted(미보고)) or "(없음)"))
    else:
    ...
    R.표(["구성", "통과", "실패", "타임아웃", "커버리지 %"],
        [[json.dumps(x["파라"], ensure_ascii=False), f"{x['pass']:,}", x["fail"], x["timeout"],
          ("-" if x.get('cov') is None else f"{x['cov']:.1f}")] for x in m["구성"]],
    ...
    R.잰것 = [
        ...
        # **안 잰 것을 0 으로 적지 않는다.** 커버리지를 안 내는 회로는 '-' 로 두고
        # 무엇으로 쟀는지 칸에 그 까닭을 적는다.
        ("기능 커버리지(최종)",
         (f"{끝cov:.2f}" if 커버있나 else "-"), "%",
         ("커버그룹 6종 + 크로스 2종" if 커버있나 else "이 회로의 TB 는 커버리지를 안 낸다")),
        ("자해 검사 잡음/전체", f"{잡힌}/{len(변)}", "종", "house/dv/mutate.py"),
        ("시뮬레이션 속도", f"{m['총주기']/max(m['회귀초'],1e-9)/1e6:.2f}", "M주기/초", "실측"),
        ("회귀 소요", m["회귀초"], "s", "실측"),
        ("변이 검사 소요", m["변이초"], "s", "실측"),
        # RTL 보고서와 같은 자리다 -- 이 수는 보고서를 조립한 시간이 아니라
        # **도구가 돈 시간**이다. 표지에도 같은 수가 「Tool runtime」으로 간다.
        ("에이전트 실행 시간 (도구 포함)", m["초"], "s", "실측"),
    ]
    return R
