    ...
    for i, (항, 값) in enumerate(list(못박힌수 or []), 1):
        ...
        꼴 = {
            ...
            "길이": (id앞, f"{항}={값}: 최소 · 정확히 · 넘김",
                   "길이를 정하는 수는 <b>딱 맞을 때</b>와 <b>넘을 때</b>가 다른 "
            ...
            "개수": (id앞, f"{항}={값}: 하나만 · 전부 · 마지막 하나",
                   "여럿을 두는 수는 <b>하나만 쓸 때</b>와 <b>전부 동시에 쓸 때</b>가 "
                   "다르다. 마지막 하나가 인덱스 경계다",
...
def 세우기(설계=None, 훑기=None, 스펙=None) -> dict:
    ...
    for v, r, 기 in 짝:
        ...
        if r:
            ...
            적기(f"hs_{꼬}_hold", f"{꼬}: valid 유지 규칙",
               ...
               "데이터와 valid 가 변하면 안 된다. <b>프로토콜 위반은 0 이어야 한다</b>",
    ...
    for k, v in (훑.get("톱파라미터") or {}).items():
        ...
        if any(w in 낮k for w in _단수말):
            적기(f"pipe_{k}", f"{k}={v}: 파이프라인 채움과 비움",
               "단수를 정하는 파라미터는 <b>채우는 동안</b>과 <b>비우는 동안</b>에 "
    ...
    if 오류포트 or re.search(r"overflow|sticky|error flag|오버플로|에러 플래그", 스펙글, re.I):
        적기("err_inject", "오류 플래그: 세우는 조건과 안 세우는 조건",
           "<b>플래그가 서는 것만 보면 절반만 검사한 것이다.</b> 안 서야 할 때 "
