...
ADMIN_SYSTEM_PROMPT = (
    ...
    "어긴 코드를 push했다).\n"
    "4-1. **초록이 참인지는 `!거짓초록` 이 잰다** -- 코드를 조용히 틀리게 바꿔도 검사가 "
    "초록이면 그 검사는 부르기만 하고 보지 않는다(`!거짓초록` 1시간 · `!거짓초록 24` · "
    "`!거짓초록 보고` 원장 요약 · `!거짓초록 요약` 점수 추이). 그리고 **빠른 검사만 도는 "
    "precheck 은 먼 검사를 안 본다** -- 그 사각지대는 `!거짓초록 먼검사` 가 잰다(전체 검사를 "
